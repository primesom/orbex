...
import orbex.api
import orbex.modules.neutralize
import orbex.release
import orbex.sql_db
import orbex.tools
from orbex.exceptions import AccessDenied
from orbex.release import version_info
from orbex.sql_db import db_connect
from orbex.tools import osutil, SQL
from orbex.tools.misc import exec_pg_environ, find_pg_tool
...
def check_super(passwd):
    if passwd and orbex.tools.config.verify_admin_password(passwd):
        return True
    raise orbex.exceptions.AccessDenied()


# This should be moved to orbex.modules.db, along side initialize().
def _initialize_db(db_name, demo, lang, user_password, login='admin', country_code=None, phone=None):
    try:
        orbex.tools.config['load_language'] = lang

        registry = orbex.modules.registry.Registry.new(db_name, update_module=True, new_db_demo=demo)

        with closing(registry.cursor()) as cr:
            env = orbex.api.Environment(cr, orbex.api.SUPERUSER_ID, {})
            ...
            if login:
                values['login'] = login
                emails = orbex.tools.email_split(login)
                if emails:
...
def _check_faketime_mode(db_name):
    if os.getenv('orbex_FAKETIME_TEST_MODE') and db_name in orbex.tools.config['db_name']:
        try:
            db = orbex.sql_db.db_connect(db_name)
...
def _create_empty_database(name):
    db = orbex.sql_db.db_connect('postgres')
    with closing(db.cursor()) as cr:
        chosen_template = orbex.tools.config['db_template']
    ...
    try:
        db = orbex.sql_db.db_connect(name)
        with db.cursor() as cr:
            cr.execute("CREATE EXTENSION IF NOT EXISTS pg_trgm")
            if orbex.tools.config['unaccent']:
                ...
                # But in the case of orbex, we consider that those data don't
...
def exp_duplicate_database(db_original_name, db_name, neutralize_database=False):
    ...
    orbex.sql_db.close_db(db_original_name)
    db = orbex.sql_db.db_connect('postgres')
    with closing(db.cursor()) as cr:
        # database-altering operations cannot be executed inside a transaction
        cr._cnx.autocommit = True
    ...
    registry = orbex.modules.registry.Registry.new(db_name)
    with registry.cursor() as cr:
        # if it's a copy of a database, force generation of a new dbuuid
        env = orbex.api.Environment(cr, orbex.api.SUPERUSER_ID, {})
        env['ir.config_parameter'].init(force=True)
        if neutralize_database:
            orbex.modules.neutralize.neutralize_database(cr)

    from_fs = orbex.tools.config.filestore(db_original_name)
    to_fs = orbex.tools.config.filestore(db_name)
...
def exp_drop(db_name):
    if db_name not in list_dbs(True):
        return False
    orbex.modules.registry.Registry.delete(db_name)
    orbex.sql_db.close_db(db_name)
    ...
    fs = orbex.tools.config.filestore(db_name)
...
def dump_db_manifest(cr):
    ...
    manifest = {
        'orbex_dump': '1',
        'db_name': cr.dbname,
        'version': orbex.release.version,
        'version_info': orbex.release.version_info,
        'major_version': orbex.release.major_version,
...
def dump_db(db_name, stream, backup_format='zip', with_filestore=True):
    ...
    if backup_format == 'zip':
        with tempfile.TemporaryDirectory() as dump_dir:
            if with_filestore:
                filestore = orbex.tools.config.filestore(db_name)
...
def restore_db(db, dump_file, copy=False, neutralize_database=False):
    ...
    with tempfile.TemporaryDirectory() as dump_dir:
        ...
        registry = orbex.modules.registry.Registry.new(db)
        with registry.cursor() as cr:
            env = orbex.api.Environment(cr, orbex.api.SUPERUSER_ID, {})
            if copy:
                # if it's a copy of a database, force generation of a new dbuuid
...
def exp_rename(old_name, new_name):
    orbex.modules.registry.Registry.delete(old_name)
    orbex.sql_db.close_db(old_name)
    ...
    old_fs = orbex.tools.config.filestore(old_name)
    new_fs = orbex.tools.config.filestore(new_name)
...
def exp_change_admin_password(new_password):
    orbex.tools.config.set_admin_password(new_password)
    orbex.tools.config.save(['admin_passwd'])
    return True
...
def exp_migrate_databases(databases):
    for db in databases:
        ...
        orbex.modules.registry.Registry.new(db, update_module=True, upgrade_modules={'base'})
    return True
...
@orbex.tools.mute_logger('orbex.sql_db')
...
def list_dbs(force=False):
    if not orbex.tools.config['list_db'] and not force:
        raise orbex.exceptions.AccessDenied()

    if not orbex.tools.config['dbfilter'] and orbex.tools.config['db_name']:
        # In case --db-filter is not provided and --database is passed, orbex will not
        ...
        return sorted(orbex.tools.config['db_name'])
...
def list_db_incompatible(databases):
    """"Check a list of databases if they are compatible with this version of orbex
    ...
    for database_name in databases:
        with closing(db_connect(database_name).cursor()) as cr:
            if orbex.tools.sql.table_exists(cr, 'ir_module_module'):
    ...
    for database_name in incompatible_databases:
        # release connection
        orbex.sql_db.close_db(database_name)
...
def exp_list_lang():
    return orbex.tools.misc.scan_languages()
