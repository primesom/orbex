...
import orbex.cli
from orbex.modules import initialize_sys_path, load_script
...
def load_internal_commands():
    """ Load ``commands`` from ``orbex.cli`` """
    for path in orbex.cli.__path__:
        for module in Path(path).iterdir():
            if module.suffix != '.py':
                continue
            __import__(f'orbex.cli.{module.stem}')
    ...
    for path in orbex.addons.__path__:
        for fullpath in Path(path).glob(f'*/cli/{command}.py'):
            if (found_command := fullpath.stem) and Command.is_valid_name(found_command):
                # loading as orbex.cli and not orbex.addons.{module}.cli
                # so it doesn't load orbex.addons.{module}.__init__
                mapping[f'orbex.cli.{found_command}'] = fullpath 
...
def find_command(name: str) -> Command | None:
    ...
    # import from orbex.cli
    with contextlib.suppress(ImportError):
        __import__(f'orbex.cli.{name}')
        return commands[name]

    # import from orbex.addons.*.cli
...
def main():
    ...
    if command := find_command(command_name):
        orbex.cli.COMMAND = command_name
        command().run(args)
    else:
        message = (
