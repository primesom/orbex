...
class Neutralize(Command):
    ...
    def run(self, args):
        parser = orbex.tools.config.parser
        ...
        opt = orbex.tools.config.parse_config(args, setup_logging=True)
        ...
        try:
            with orbex.sql_db.db_connect(dbname).cursor() as cursor:
                if opt.to_stdout:
                    installed_modules = orbex.modules.neutralize.get_installed_modules(cursor)
                    queries = orbex.modules.neutralize.get_neutralization_queries(installed_modules)
                    ...
                    orbex.modules.neutralize.neutralize_database(cursor)
