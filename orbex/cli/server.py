...
from orbex.release import author as __author__  # noqa: F401
from orbex.release import version as __version__  # noqa: F401
from orbex.service import server
from orbex.tools import config

from . import Command

# Also use the `orbex` logger for the main script.
_logger = logging.getLogger('orbex')
...
def report_configuration():
    ...
    import orbex.release  # noqa: PLC0415
    _logger.info("orbex version %s", orbex.release.version)
    if os.path.isfile(config['config']):
        _logger.info("Using configuration file at %s", config['config'])
    _logger.info('addons paths: %s', orbex.addons.__path__)
    ...
    if sys.version_info[:2] > orbex.release.MAX_PY_VERSION:
        _logger.warning("Python %s is not officially supported, please use Python %s instead",
            '.'.join(map(str, sys.version_info[:2])),
            '.'.join(map(str, orbex.release.MAX_PY_VERSION))
        )
...
def setup_pid_file():
    ...
    This function assumes the configuration has been initialized.
    """
    import orbex  # for evented  # noqa: PLC0415
    if not orbex.evented and config['pidfile']:
        pid = os.getpid()
...
def main(args):
    ...
    for db_name in config['db_name']:
        from orbex.service import db  # noqa: PLC0415
        try:
...
class Server(Command):
    """Start the orbex server (default command)"""
