...
from orbex import SUPERUSER_ID
from orbex.api import Environment
from orbex.cli.command import Command
from orbex.modules.loading import force_demo
from orbex.modules.module import get_module_path, initialize_sys_path
from orbex.modules.registry import Registry
from orbex.tools import OrderedSet, config, parse_version
...
class Module(Command):
    ...
    def __init__(self, *args, **kwargs):
        ...
        install_parser.epilog = textwrap.dedent("""\
            Before installing modules, an orbex database needs to be created and initialized
            on your PostgreSQL instance, using the `db init` command:

            $ orbex-bin db init <db_name>

            To get help on its parameters, see:

            $ orbex-bin db init --help
        """)
