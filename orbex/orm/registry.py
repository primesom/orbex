...
from orbex.tools.func import locked, reset_cached_properties
from orbex.tools.lru import LRU
from orbex.tools.misc import Collector, format_frame
...
if typing.TYPE_CHECKING:
    ...
    from orbex.fields import Field
    from orbex.models import BaseModel
    from orbex.sql_db import BaseCursor, Connection, Cursor
    from orbex.modules import module_graph


_logger = logging.getLogger('orbex.registry')
_schema = logging.getLogger('orbex.schema')
...
class Registry(Mapping[str, type["BaseModel"]]):
        ...
        *,
        ...
        try:
            ...
            with registry.cursor() as cr:
                ...
                from orbex.modules import db  # noqa: PLC0415
                if db.is_initialized(cr):
                    cr.execute("DELETE FROM ir_config_parameter WHERE key='base.partially_updated_database'")
                    if cr.rowcount:
                        update_module = True
            # This should be a method on Registry
            from orbex.modules.loading import load_modules, reset_modules_state  # noqa: PLC0415
    ...
    def init(self, db_name: str) -> None:
        ...
        if config['test_enable']:
            from orbex.tests.result import orbexTestResult  # noqa: PLC0415
            self._assertion_report: orbexTestResult | None = orbexTestResult()
        else:
        ...
        for indexname, tablename, field in expected:
            ...
            if index and indexname not in existing:
                ...
                if index == 'trigram':
                    ...
                    from orbex.modules.db import FunctionStatus  # noqa: PLC0415
