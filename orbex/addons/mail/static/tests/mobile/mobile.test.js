...
import { describe, test } from "@orbex/hoot";
import { advanceTime, pointerDown, press } from "@orbex/hoot-dom";
import { Deferred, mockTouch, mockUserAgent } from "@orbex/hoot-mock";
...
test("click on an orbex link should fold the chat window (mobile)", async () => {
    const pyEnv = await startServer();
    const channelId = pyEnv["discuss.channel"].create({});
    patchUiSize({ size: SIZES.SM });
    await start();
    await openDiscuss(channelId);
    await insertText(".o-mail-Composer-input", `http://${browser.location.host}/orbexsuite.com`);
    await click(".o-mail-Composer button[title='Send']");
    await contains(".o-mail-ChatWindow");
    await click(`a[href="http://${browser.location.host}/orbexsuite.com"]`);
