...
from orbex import tools, fields
from orbex.addons.base.models.ir_mail_server import IrMail_Server
from orbex.addons.base.tests.common import MockSmtplibCase
from orbex.addons.bus.models.bus import BusBus, json_dump
from orbex.addons.bus.tests.common import BusCase
from orbex.addons.mail.models import mail_thread
from orbex.addons.mail.models.mail_mail import MailMail
from orbex.addons.mail.models.mail_message import MailMessage
from orbex.addons.mail.models.mail_notification import MailNotification
from orbex.addons.mail.models.res_users import ResUsers
from orbex.addons.mail.tools.discuss import Store
from orbex.tests import common, RecordCapturer, new_test_user
from orbex.tools import mute_logger
from orbex.tools.mail import (
    email_normalize, email_normalize_all, email_split, email_split_and_format_normalize, formataddr
)
from orbex.tools.translate import code_translations
...
class MockEmail(common.BaseCase, MockSmtplibCase):
    ...
    def mock_push_to_end_point(self, max_direct_push=5):
        with patch.object(mail_thread, 'push_to_end_point') as patched_push, \
             patch('orbex.addons.mail.models.mail_thread.MAX_DIRECT_PUSH', max_direct_push):
    ...
    def assertNotSentEmail(self, recipients=None, message_id=None):
        ...
        :param message_id:
            ...
            from the a specific message in orbex.
        """
        mails = self._mails
        if message_id:
...
class MailCommon(MailCase):
    ...
    def _activate_multi_lang(cls, lang_code='es_ES', layout_arch_db=None, test_record=False, test_template=False):
        ...
        with mute_logger("orbex.addons.base.models.ir_module", "orbex.tools.translate"):
...
<body>
    ...
    def _setup_push_devices_for_partners(cls, partners, endpoint=None):
        ...
        endpoint = endpoint or "https://test.orbexsuite.com/webpush/user"
...
def freeze_all_time(dt=None):
    ...
    with patch('orbex.sql_db.BaseCursor.now', return_value=dt), freeze_time(dt):
        yield
