...
import { highlightText, orbexmark } from "@web/core/utils/html";
...
export class Many2XAutocomplete extends Component {
    ...
    onQuickCreateError(error, request) {
        if (
            ...
            error.exceptionName === "orbex.exceptions.ValidationError"
        ) {
    ...
    buildRecordSuggestion(request, record) {
        ...
        return {
            ...
            label: label
                ? highlightText(request, orbexmark(label), "text-primary fw-bold")
                : _t("Unnamed"),
