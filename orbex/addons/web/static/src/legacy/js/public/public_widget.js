...
function OrbexEvent(target, name, data) {
    this.target = target;
    this.name = name;
...
OrbexEvent.prototype.stopPropagation = function () {
    this.stopped = true;
};
OrbexEvent.prototype.is_stopped = function () {
    return this.stopped;
};
...
const EventDispatcherMixin = Object.assign({}, ParentedMixin, {
     ...
     * Orbex where developers may want to replace existing callbacks with theirs.
     *
    ...
    "trigger_up": function (name, info) {
        var event = new OrbexEvent(this, name, info);
