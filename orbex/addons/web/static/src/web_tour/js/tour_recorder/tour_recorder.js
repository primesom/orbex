...
import { queryAll, queryFirst, queryOne } from "@orbex/hoot-dom";
import { Component, useState, useExternalListener } from "@orbex/owl";
...
const ORBEX_CLASS_REGEX = /^oe?(-|_)[\w-]+$/;
...
const getShortestSelector = (paths) => {
    paths.reverse();
    let filteredPath = [];
    let hasOrbexClass = false;
    for (
        let currentElem = paths.pop();
        (currentElem && queryAll(filteredPath.join(" > ")).length !== 1) || !hasOrbexClass;
        ...
        const orbexClass = [...currentElem.classList].find((c) => c.match(ORBEX_CLASS_REGEX));
        if (orbexClass) {
            currentPredicate = `.${orbexClass}`;
            hasOrbexClass = true;
        }
