import { Component, markup, whenReady, validate } from "@orbex/owl";
import { browser } from "@web/core/browser/browser";
import { DropdownItem } from "@web/core/dropdown/dropdown_item";
import { registry } from "@web/core/registry";
import { session } from "@web/session";
import { loadBundle } from "@web/core/assets";
import { createPointerState } from "@web/web_tour/js/tour_pointer/tour_pointer_state";
import { tourState } from "@web/web_tour/js/tour_state";
import { callWithUnloadCheck } from "@web/web_tour/js/utils/tour_utils";
import {
    tourRecorderState,
    TOUR_RECORDER_ACTIVE_LOCAL_STORAGE_KEY,
} from "@web/web_tour/js/tour_recorder/tour_recorder_state";
    ...
    start: async (env, { orm, effect, overlay }) => {
        ...
        async function resumeTour() {
            ...
            if (tourConfig.mode === "auto") {
                if (!orbex.loader.modules.get("@web/web_tour/js/tour_automatic/tour_automatic")) {
                    await loadBundle("web.tour_automatic", { css: false });
                }
                const { TourAutomatic } = orbex.loader.modules.get(
                    "@web/web_tour/js/tour_automatic/tour_automatic"
                );
                new TourAutomatic(tour).start();
            } else {
                await loadBundle("web.tour_interactive");
                const { TourPointer } = orbex.loader.modules.get(
                    "@web/web_tour/js/tour_pointer/tour_pointer"
                );
                ...
                const { TourInteractive } = orbex.loader.modules.get(
                    "@web/web_tour/js/tour_interactive/tour_interactive"
                );
        ...
        async function tourRecorder() {
            await loadBundle("web.tour_recorder");
            const { TourRecorder } = orbex.loader.modules.get(
                "@web/web_tour/js/tour_recorder/tour_recorder"
            );
        ...
        orbex.startTour = startTour;
        orbex.isTourReady = (tourName) => getTourFromRegistry(tourName).wait_for.then(() => true);
