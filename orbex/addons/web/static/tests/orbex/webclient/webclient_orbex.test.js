...
import { click, keyDown, queryAll, queryFirst } from "@orbex/hoot-dom";
import { animationFrame, Deferred, mockMatchMedia } from "@orbex/hoot-mock";
...
import { shareUrlMenuItem } from "@web/orbex/webclient/share_url/share_url";
...
// Should test ONLY the webClient and features present in Orbex
...
describe("basic flow with home menu", () => {
    ...
    test("1 -- start up", async () => {
        await mountWebClient({ WebClient: WebClientOrbex });
...
test("initial action crashes", async () => {
    expect.errors(1);
    redirect("/orbex/action-__test__client__action__?menu_id=1");
...
test("Navigate to an application from the HomeMenu should generate only one pushState", async () => {
    ...
    await animationFrame();
    expect(".o_home_menu").toHaveCount(1);
    expect.verifySteps(["/orbex", "/orbex/action-1002", "/orbex", "/orbex/action-1001", "/orbex"]);
});
...
test("display studio icon when studio module is not installed", async () => {
    ...
    expect(queryFirst(".modal-header").textContent).toBe(
        "orbex Studio - Add new fields to any view"
    );
});
