...
test("test reload client action", async () => {
    ...
    await runAllTimers();
    expect.verifySteps([
        "replaceState /orbex?test=42",
        "window_reload",
        "pushState /orbex/action-2",
        "window_reload",
        "pushState /orbex?menu_id=1",
        "window_reload",
        "pushState /orbex/action-1?menu_id=2",
        "window_reload",
    ]);
});
...
test("test display_exception client action", async () => {
    ...
    getService("action").doAction({
        ...
        params: {
            ...
            data: {
                name: `orbex.exceptions.UserError`,
                debug: "traceback",
                arguments: [],
                context: {},
