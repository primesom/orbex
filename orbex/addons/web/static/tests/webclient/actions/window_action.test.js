...
import { click, queryAllTexts, waitFor } from "@orbex/hoot-dom";
import { Deferred, animationFrame, runAllTimers } from "@orbex/hoot-mock";
...
test("A new form view can be reloaded after a failed one", async () => {
    ...
    await runAllTimers(); // wait for the update of the router
    expect(browser.location.pathname).toBe("/orbex/action-3/1");
    ...
    await runAllTimers(); // wait for the update of the router
    expect(browser.location.pathname).toBe("/orbex/action-3/2");
