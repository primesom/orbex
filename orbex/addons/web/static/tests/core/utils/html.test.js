...
import { htmlEscape, markup } from "@orbex/owl";
...
test("orbexmark", () => {
    expect(orbexmark("").toString()).toBe("");
    expect(orbexmark("**test**").toString()).toBe("<b>test</b>");
    expect(orbexmark("**test** something else **test**").toString()).toBe(
        "<b>test</b> something else <b>test</b>"
    );
    expect(orbexmark("--test--").toString()).toBe(`<span class="text-muted">test</span>`);
    expect(orbexmark("--test-- something else --test--").toString()).toBe(
        `<span class="text-muted">test</span> something else <span class="text-muted">test</span>`
    );
    expect(orbexmark("`test`").toString()).toBe(
    ...
    expect(orbexmark("test\ttest2").toString()).toBe(
        `test<span style="margin-left: 2em"></span>test2`
    );
    expect(orbexmark("test\ntest2").toString()).toBe("test<br>test2");
    expect(orbexmark("<p>**test**</p>").toString()).toBe("&lt;p&gt;<b>test</b>&lt;/p&gt;");
    expect(orbexmark(markup`<p>**test**</p>`).toString()).toBe("<p><b>test</b></p>");
});
