...
from orbex.http import content_disposition, dispatch_rpc, request, Response
from orbex.service import db
from orbex.tools.misc import file_open, str2bool
from orbex.tools.translate import _

from orbex.addons.base.models.ir_qweb import render as qweb_render
...
class Database(http.Controller):
    ...
    def _render_template(self, **d):
        ...
        d['insecure'] = orbex.tools.config.verify_admin_password('admin')
        d['list_db'] = orbex.tools.config['list_db']
        d['langs'] = orbex.service.db.exp_list_lang()
        d['countries'] = orbex.service.db.exp_list_countries()
        d['pattern'] = DBNAME_PATTERN
        # databases list
        try:
            d['databases'] = http.db_list()
            d['incompatible_databases'] = orbex.service.db.list_db_incompatible(d['databases'])
    ...
    def create(self, master_pwd, name, lang, password, **post):
        insecure = orbex.tools.config.verify_admin_password('admin')
        ...
        try:
            ...
            with orbex.modules.registry.Registry(name).cursor() as cr:
    ...
    def backup(self, master_pwd, name, backup_format='zip', filestore=True):
        ...
        if insecure and master_pwd:
            dispatch_rpc('db', 'change_admin_password', ["admin", master_pwd])
        try:
            orbex.service.db.check_super(master_pwd)
            ...
            dump_stream = orbex.service.db.dump_db(name, None, backup_format, filestore)
