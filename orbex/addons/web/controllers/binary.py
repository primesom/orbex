...
try:
    ...
    from orbex.tools._vendor.send_file import send_file

import orbex
import orbex.modules.registry
from orbex import SUPERUSER_ID, _, http, api
from orbex.addons.base.models.assetsbundle import ANY_UNIQUE
from orbex.exceptions import AccessError, UserError
from orbex.http import request, Response
from orbex.tools import file_open, file_path, replace_exceptions, str2bool
from orbex.tools.image import image_guess_size_from_field_name
from orbex.tools.mimetypes import guess_mimetype

_logger = logging.getLogger(__name__)

BAD_X_SENDFILE_ERROR = """\
Orbex is running with --x-sendfile but is receiving /web/filestore requests.

With --x-sendfile enabled, NGINX should be serving the
/web/filestore route, however Orbex is receiving the
request.
...
class Binary(http.Controller):
    ...
    def content_filestore(self, _path):
        if orbex.tools.config['x_sendfile']:
            # pylint: disable=logging-format-interpolation
            _logger.error(BAD_X_SENDFILE_ERROR.format(
                data_dir=orbex.tools.config['data_dir']
            ))
    ...
    def company_logo(self, dbname=None, **kw):
        ...
        imgext = '.png'
        dbname = request.db
        uid = (request.session.uid if dbname else None) or orbex.SUPERUSER_ID
            ...
            try:
                ...
                if company:
                    request.env.cr.execute("""
                        SELECT logo_web, write_date, uses_default_logo
                          FROM res_company
                         WHERE id = %s
                    """, (company,))
                else:
                    request.env.cr.execute("""
                        SELECT c.logo_web, c.write_date, c.uses_default_logo
                          FROM res_users u
                ...
                if row and row[0] and not row[2]:
                    ...
                    response = http.Stream.from_path(file_path('base/static/img/res_company_logo.png')).get_response()
            except Exception:
                _logger.warning("While retrieving the company logo, using the Orbex logo instead", exc_info=True)
