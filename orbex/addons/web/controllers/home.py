...
class Home(http.Controller):
    ...
    def index(self, s_action=None, db=None, **kw):
        ...
        return request.redirect_query('/orbex', query=request.params)

    def _web_client_readonly(self, rule, args):
        return False

    # ideally, this route should be `auth="user"` but that don't work in non-monodb mode.
    @http.route(['/web', '/orbex', '/orbex/<path:subpath>', '/scoped_app/<path:subpath>'], type='http', auth="none", readonly=_web_client_readonly)
    def web_client(self, s_action=None, **kw):
        if kw.get('subpath') == 'discuss':
            query = {key: value for key, value in request.params.items() if key != 'subpath'}
            return request.redirect_query('/orbex/chat', query=query, code=301)
