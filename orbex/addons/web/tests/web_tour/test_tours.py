...
from orbex.addons.base.tests.common import BaseCommon, HttpCase
...
class WebTourHttp(HttpCase):
    ...
    def setUpClass(cls):
        super().setUpClass()
        cls.eager_files = ["/web/static/src/web_tour/js/tour_automatic/tour_helpers.js"]
    ...
    def test_sanity_automatic(self):
        ...
        tour_auto_bundle = IrAsset._get_asset_paths("web.tour_automatic", {})
        ...
        code = """
        orbex.define("@web/web_tour/../tests/sanity_test", [], () => {
            const errors = [];
            for (const module of ["@orbex/hoot-dom", "@web/web_tour/js/tour_step"]) {
                if (orbex.loader.modules.get(module)) {
                    errors.push(module)
                }
            }
            if (!errors.length) {
        ...
        """
        self.browser_js("/orbex?debug=tests", code, ready=ready, login="admin")
    ...
    def test_sanity_onboarding(self):
        ...
        # We want to boot Orbex as in real life (not loading assets for tests)
        ...
        """
        self.browser_js("/orbex?debug=0", code, ready=ready, login="admin")
        if "website" in IrAsset._get_installed_addons_list():
