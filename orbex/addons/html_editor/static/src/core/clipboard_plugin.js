...
export class ClipboardPlugin extends Plugin {
    ...
    onPaste(ev) {
        ...
        if (!this.delegateTo("paste_overrides", selection, ev.clipboardData)) {
            this.handlePasteUnsupportedHtml(selection, ev.clipboardData) ||
                this.handlePasteOrbexEditorHtml(ev.clipboardData) ||
     ...
     * @param {DataTransfer} clipboardData
     */
    handlePasteOrbexEditorHtml(clipboardData) {
        const orbexEditorHtml = clipboardData.getData("application/vnd.orbex.orbex-editor");
        const textContent = clipboardData.getData("text/plain");
        if (ONLY_LINK_REGEX.test(textContent)) {
            return false;
        }
        if (orbexEditorHtml) {
            const fragment = parseHTML(this.document, orbexEditorHtml);
    ...
    async onDrop(ev) {
        ...
        const orbexEditorHtml = ev.dataTransfer.getData("application/vnd.orbex.orbex-editor");
        const fileTransferItems = !orbexEditorHtml && getImageFiles(dataTransfer);
        const htmlTransferItem = [...dataTransfer.items].find((item) => item.type === "text/html");
        if (fileTransferItems.length || htmlTransferItem || orbexEditorHtml) {
