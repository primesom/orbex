...
import orbex.http
from orbex.tests import tagged, get_db_name
from orbex.tests.common import new_test_user, Like
from orbex.tools import mute_logger
from orbex.tools.misc import submap
from orbex.addons.test_http.utils import HtmlTokenizer
...
class TestHttpModels(TestHttpBase):
    ...
    def test_models3_stargate_ko(self):
        ...
        self.assertEqual(logs.output, ["WARNING:orbex.http:The goauld destroyed the gate"])
    ...
    def test_models4_stargate_setname(self):
        ...
        res = self.url_open(f'/test_http/{milky_way.id}/setname?readonly=0', {
            'name': "Wilky May",
            'csrf_token': orbex.http.Request.csrf_token(self),
        })
    ...
    def test_models5_stargate_setname_readonly(self):
        ...
        with self.assertLogs('orbex.http', 'WARNING') as capture_http,\
             self.assertLogs('orbex.sql_db', 'WARNING') as capture_sql_db:
    ...
    def test_models6_rpc_path_poisoning(self):
        with self.assertLogs('werkzeug', logging.INFO) as capture:
            with mute_logger('orbex.addons.rpc.controllers.xmlrpc'):
