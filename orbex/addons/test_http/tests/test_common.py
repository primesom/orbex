...
from orbex.http import Session
from orbex.addons.base.tests.common import HttpCaseWithUserDemo
from orbex.tools import config, reset_cached_properties
from orbex.addons.test_http.utils import MemoryGeoipResolver, MemorySessionStore
...
class TestHttpBase(HttpCaseWithUserDemo):
    ...
    def setUpClass(cls):
        ...
        cls.classPatch(orbex.http.Application, 'session_store', session_store)
        cls.classPatch(orbex.http.Application, 'geoip_city_db', geoip_resolver)
        cls.classPatch(orbex.http.Application, 'geoip_country_db', geoip_resolver)

    def setUp(self):
        super().setUp()
        orbex.http.root.session_store.store.clear()
    ...
    def nodb_url_open(self, url, *args, allow_redirects=False, **kwargs):
        with patch('orbex.http.db_list') as db_list, \
             patch('orbex.http.db_filter') as db_filter:
    ...
    def multidb_url_open(self, url, *args, allow_redirects=False, dblist=(), **kwargs):
        ...
        with patch('orbex.http.db_list') as db_list, \
             patch('orbex.http.db_filter') as db_filter, \
             patch('orbex.http.Registry') as Registry:
