...
QWeb is the primary templating engine used by Orbex. It is an XML templating
...
<https://www.orbexsuite.com/documentation/master/developer/reference/frontend/qweb.html>`
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
**Values**: name of the allowed orbex user group, or preceded by ``!`` for
prohibited groups

The generated code uses ``has_group`` Orbex method from ``res.users`` model.
...
from orbex.modules.registry import _REGISTRY_CACHES
from orbex.tools import config, safe_eval, OrderedSet, frozendict, json
from orbex.tools.constants import SUPPORTED_DEBUGGER, EXTERNAL_ASSET
from orbex.tools.safe_eval import assert_valid_codeobj, _BUILTINS, to_opcodes, _EXPR_OPCODES, _BLACKLIST
from orbex.tools.lru import LRU
from orbex.tools.misc import str2bool, file_open, file_path
from orbex.tools.image import image_data_uri, FILETYPE_BASE64_MAGICWORD
from orbex.tools.profiler import ExecutionContext
from orbex.tools.translate import FORMAT_REGEX
from orbex.http import request
from orbex.tools.profiler import QwebTracker
from orbex.exceptions import UserError, MissingError

from orbex.addons.base.models.assetsbundle import AssetsBundle
from orbex.tools.constants import SCRIPT_EXTENSIONS, STYLE_EXTENSIONS, TEMPLATE_EXTENSIONS
    ...
    * to customize ``t-field`` rendering, subclass ``ir.qweb.field`` and
    ...
    def _render_iterall(self, view_ref, method, values, directive='render') -> Iterator[str]:
        ...
        try:
            ...
            if hasattr(error, 'qweb'):
                ...
                # If is not an orbex Exception check if the current error is raise from
    ...
    def _debug_trace(self, debugger, values):
        ...
        if not debugger:
            ...
            warnings.warn(
                ...
                "since Orbex 17.0, keep the value empty and configure the "
    ...
    def _link_to_node(self, path, defer_load=False, lazy_load=False, media=None):
        ...
        if is_js:
            ...
            if is_asset_bundle:
                attributes['onerror'] = "__orbexAssetError=1"
