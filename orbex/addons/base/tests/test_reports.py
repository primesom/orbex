...
@orbex.tests.tagged('post_install', '-at_install', 'post_install_l10n')
class TestReports(orbex.tests.TransactionCase):
    def test_reports(self):
        ...
        specific_model_domains = {
            ...
            'account_edi_ubl_cii.account_invoices_generated_by_orbex': [('move_type', 'in', ('in_invoice', 'in_refund'))],
    ...
    def test_merge_pdfs(self):
        ...
        with self.assertRaises(UserError, msg="Orbex is unable to merge the generated PDFs."):
        ...
        minimal_file = tools.pdf.OrbexPdfFileReader(minimal_pdf_content)
        merged_file = tools.pdf.OrbexPdfFileReader(merged_pdf)
...
class TestReportsRenderingCommon(orbex.tests.HttpCase):
    ...
    def save_pdf(self):
        ...
        orbex.tests.save_test_file(self._testMethodName, self.last_pdf_content, 'pdf_', 'pdf', document_type='Report PDF', logger=_logger)
...
@orbex.tests.tagged('post_install', '-at_install', 'pdf_rendering')
...
@orbex.tests.tagged('post_install', '-at_install', '-standard', 'pdf_rendering')
...
class TestAggregatePdfReports(orbex.tests.HttpCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
