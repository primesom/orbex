...
from orbex.addons.base.tests import mail_examples
...
class TestIrMailServer(TransactionCase, MockSmtplibCase):
    ...
    def test_mail_server_priorities(self):
        ...
        simple Orbex DB so we have to spoof the FROM otherwise we cannot send
        any email. """
    ...
    @mute_logger('orbex.models.unlink', 'orbex.addons.base.models.ir_mail_server')
    ...
    def test_mail_server_config_bin(self):
        """ Test the configuration provided in the orbex-bin arguments. This config
        ...
        orbex-bin argument "--from-filter".
        """
        IrMailServer = self.env['ir.mail_server']

        # Remove all mail server so we will use the orbex-bin arguments
        ...
        # for from_filter in ICP, overwrite the one from orbex-bin
    ...
    def test_mail_server_config_cli(self):
        ...
        "cli". It should take the configuration from the orbex-bin argument. The
        ...
        for mail_from, (expected_smtp_from, expected_msg_from, expected_mail_server) in zip(
            [
                # check that the CLI server take the configuration in the orbex-bin argument
