...
import orbex.tools as tools
import orbex.upgrade
...
class UpgradeHook:
    """Makes the legacy `migrations` package being `orbex.upgrade`"""

    def find_spec(self, fullname, path=None, target=None):
        if re.match(r"^orbex\.addons\.base\.maintenance\.migrations\b", fullname):
    ...
    def load_module(self, name):
        ...
        canonical_upgrade = name.replace("orbex.addons.base.maintenance.migrations", "orbex.upgrade")
...
def initialize_sys_path() -> None:
    """
    Setup the addons path ``orbex.addons.__path__`` with various defaults
    ...
    for path in (
        ...
        *tools.config['addons_path'],
        ...
        if os.access(path, os.R_OK) and path not in orbex.addons.__path__:
            orbex.addons.__path__.append(path)

    # hook orbex.upgrade on upgrade-path
    ...
    for up in tools.config['upgrade_path'] or [legacy_upgrade_path]:
        if up not in orbex.upgrade.__path__:
            orbex.upgrade.__path__.append(up)

    # create decrecated module alias from orbex.addons.base.maintenance.migrations to orbex.upgrade
    spec = importlib.machinery.ModuleSpec("orbex.addons.base.maintenance", None, is_package=True)
    maintenance_pkg = importlib.util.module_from_spec(spec)
    maintenance_pkg.migrations = orbex.upgrade  # type: ignore
    sys.modules["orbex.addons.base.maintenance"] = maintenance_pkg
    sys.modules["orbex.addons.base.maintenance.migrations"] = orbex.upgrade

    # hook for upgrades and namespace freeze
    if not getattr(initialize_sys_path, 'called', False):  # only initialize once
        orbex.addons.__path__._path_finder = lambda *a: None  # prevent path invalidation
        orbex.upgrade.__path__._path_finder = lambda *a: None  # prevent path invalidation
...
class Manifest(Mapping[str, typing.Any]):
    ...
    def _get_manifest_from_addons(module: str) -> Manifest | None:
        ...
        for adp in orbex.addons.__path__:
...
def get_resource_from_path(path: str) -> tuple[str, str, str] | None:
    ...
    """
    resource = None
    sorted_paths = sorted(orbex.addons.__path__, key=len, reverse=True)
...
def load_openerp_module(module_name: str) -> None:
    ...
    qualname = f'orbex.addons.{module_name}'
