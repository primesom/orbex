...
r"""\
orbex HTTP layer / WSGI application
...
:class:`~orbex.http.Controller`: class and its companion the
:func:`~orbex.http.route`: method decorator. Together they are used to
...
Application.__call__
  ...
  request and itself in an orbex http request. The orbex http request is
...
import orbex.init  # import first for core setup
...
orbex URLs are CSRF-protected by default (when accessed with unsafe
HTTP methods). See
https://www.orbexsuite.com/documentation/master/developer/reference/addons/http.html#csrf
for more details.

* if this endpoint is accessed through orbex via py-QWeb form, embed a CSRF
...
<!-- Alternatively, use the X-orbex-Database header. -->
"""
...
SESSION_ROTATION_INTERVAL_HEADER_SKIP = "X-orbex-Skip-Session-Rotation-Interval"
...
def db_list(force=False, host=None):
    ...
    :param bool force: See :func:`~orbex.service.db.list_dbs`:
    ...
    """
    try:
        dbs = orbex.service.db.list_dbs(force)
...
def db_filter(dbs, host=None):
    ...
    if config['db_name']:
        # In case --db-filter is not provided and --database is passed, orbex will
...
def dispatch_rpc(service_name, method, params):
    ...
    """
    rpc_dispatchers = {
        'common': orbex.service.common.dispatch,
        'db': orbex.service.db.dispatch,
        'object': orbex.service.model.dispatch,
    }
...
class Stream:
    ...
    def from_path(cls, path, filter_ext=('',), public=False):
        ...
        :param path: See :func:`~orbex.tools.file_path`
        :param filter_ext: See :func:`~orbex.tools.file_path`
    ...
    def get_response(
        ...
        **send_file_kwargs
        ...
        :param send_file_kwargs: Other keyword arguments to send to
            :func:`orbex.tools._vendor.send_file.send_file` instead of
...
class Controller:
    ...
    :class:`~orbex.http.Controller` can use the :func:`~orbex.http.route`:
    ...
    :func:`~orbex.http.route`:. Please note that the decorators of all
    ...
    .. code-block:
        ...
        class GreetingController(orbex.http.Controller):
    ...
    def __init_subclass__(cls):
        ...
        if Controller in cls.__bases__:
            ...
            module = path[2] if path[:2] == ['orbex', 'addons'] else ''
...
def route(route=None, **routing):
    ...
    .. warning::
        ...
        :class:`~orbex.http.Controller` for more details.
...
def _generate_routing_rules(modules, nodb_only, converters=None):
    ...
    def is_valid(cls):
        ...
        path = cls.__module__.split('.')
        return path[:2] == ['orbex', 'addons'] and path[2] in modules
    ...
    def build_controllers():
        ...
        installed modules). Modules in this context are orbex addons.
        """
        # Controllers defined outside of orbex addons are outside of the
        # controller inheritance/extension mechanism.
        yield from (ctrl() for ctrl in Controller.children_classes.get('', []))

        # Controllers defined inside of orbex addons can be extended in
...
class GeoIP(collections.abc.Mapping):
    ...
    .. note:
        ...
        :attr:`~orbex.http.request.geoip`.

    .. code-block:

        >>> GeoIP('127.0.0.1').country.iso_code
        >>> orbex_ip = socket.gethostbyname('orbexsuite.com')
        >>> GeoIP(orbex_ip).country.iso_code
        'FR'
    """
...
class _Response(werkzeug.wrappers.Response):
    ...
    def load(cls, result, fname="<function>"):
        ...
        :returns: The created :class:`~orbex.http.Response`.
        :rtype: Response
...
class Response(Proxy):
    ...
    # orbex.http._response attributes
    load = ProxyFunc()
...
class Request:
    ...
    def _get_session_and_dbname(self):
        ...
        header_dbname = self.httprequest.headers.get('X-orbex-Database')
    ...
    def update_env(self, user=None, context=None, su=None):
        ...
        :type user: int or :class:`res.users record<~orbex.addons.base.models.res_users.ResUsers>`
    ...
    def make_response(self, data, headers=None, cookies=None, status=200):
        ...
        :rtype: :class:`~orbex.http.Response`
        """
    ...
    def _serve_db(self):
        ...
        try:
            ...
            self.env = orbex.api.Environment(cr, self.session.uid, self.session.context)
            try:
...
class HttpDispatcher(Dispatcher):
    ...
    def dispatch(self, endpoint, args):
        ...
        See :meth:`~orbex.http.Response.load` method for the compatible
...
class JsonRPCDispatcher(Dispatcher):
    ...
    def handle_error(self, exc: Exception) -> collections.abc.Callable:
        ...
        if isinstance(exc, NotFound):
            ...
            error['code'] = 100
            error['message'] = "orbex Session Expired"
...
class Application:
    ...
    def initialize(self):
        ...
        from orbex.service.server import load_server_wide_modules  # noqa: PLC0415
    ...
    def session_store(self):
        path = orbex.tools.config.session_dir
    ...
    def __call__(self, environ, start_response):
        ...
        if orbex.tools.config['proxy_mode'] and environ.get("HTTP_X_FORWARDED_HOST"):
            ...
            try:
                ...
                if self.get_static_file(httprequest.path):
                    ...
                    try:
                        ...
                        if (httprequest.path.startswith('/orbex/')
                            or httprequest.path in (
                                '/orbex', '/web', '/web/login', '/test_http/ensure_db',
                            )):
