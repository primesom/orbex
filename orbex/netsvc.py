...
def log(logger, level, prefix, msg, depth=None):
    warnings.warn(
        "orbex.netsvc.log is deprecated starting orbex 18, use normal logging APIs",
...
class PostgreSQLHandler(logging.Handler):
    ...
    def __init__(self):
        ...
        if tools.config['log_db'] != '%d':
            with contextlib.suppress(Exception), tools.mute_logger('orbex.sql_db'), sql_db.db_connect(tools.config['log_db'], allow_uri=True).cursor() as cr:
    ...
    def emit(self, record):
        ...
        if not dbname:
            return
        with contextlib.suppress(Exception), tools.mute_logger('orbex.sql_db'), sql_db.db_connect(dbname, allow_uri=True).cursor() as cr:
...
def init_logger():
    ...
    warnings.filterwarnings('ignore', category=BytesWarning, module='orbex.tools.image')
    ...
    if os.name == 'posix' and isinstance(handler, logging.StreamHandler) and (is_a_tty(handler.stream) or os.environ.get("orbex_PY_COLORS")):
...
DEFAULT_LOG_CONFIGURATION = [
    'orbex.http.rpc.request:INFO',
    'orbex.http.rpc.response:INFO',
    ':INFO',
]
PSEUDOCONFIG_MAPPER = {
    'debug_rpc_answer': ['orbex:DEBUG', 'orbex.sql_db:INFO', 'orbex.http.rpc:DEBUG'],
    'debug_rpc': ['orbex:DEBUG', 'orbex.sql_db:INFO', 'orbex.http.rpc.request:DEBUG'],
    'debug': ['orbex:DEBUG', 'orbex.sql_db:INFO'],
    'debug_sql': ['orbex.sql_db:DEBUG'],
    'info': [],
    'runbot': ['orbex:RUNBOT', 'werkzeug:WARNING'],
    'warn': ['orbex:WARNING', 'werkzeug:WARNING'],
    'error': ['orbex:ERROR', 'werkzeug:ERROR'],
    'critical': ['orbex:CRITICAL', 'werkzeug:CRITICAL'],
}
...
def showwarning_with_traceback(message, category, filename, lineno, file=None, line=None):
    ...
    for frame in traceback.extract_stack():
        if frame.name == '__call__' and frame.filename.endswith('/orbex/http.py'):
