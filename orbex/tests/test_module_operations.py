...
from orbex.tools import config, topological_sort, unique, profiler
from orbex.modules.registry import Registry
from orbex.netsvc import init_logger
from orbex.tests import standalone_tests
import orbex.tests.loader

_logger = logging.getLogger('orbex.tests.test_module_operations')
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Script for testing the install / uninstall / reinstall"
                    " cycle of orbex modules. Prefer the 'cycle' subcommand to"
    ...
    parser.add_argument("--addons-path", "-p", type=addons_path,
        help="Comma-separated list of paths to directories containing extra orbex modules")
...
def test_standalone(args):
    ...
    orbex.service.db._check_faketime_mode(args.database)  # noqa: SLF001
    ...
    for module_name in registry._init_modules:
        ...
        orbex.tests.loader.get_test_modules(module_name)
...
if __name__ == '__main__':
    ...
    if args.addons_path:
        ...
        orbex.modules.module.initialize_sys_path()
    ...
    logging.config.dictConfig({
        ...
        'loggers': {
            'orbex.modules.loading': {'level': 'CRITICAL'},
            'orbex.sql_db': {'level': 'CRITICAL'},
            'orbex.models.unlink': {'level': 'WARNING'},
            'orbex.addons.base.models.ir_model': {'level': "WARNING"},
        }
    })
