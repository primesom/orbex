...
from .result import orbexTestResult

_logger = logging.getLogger(__name__)

TEST_MODULE_NAME_PATTERN = re.compile(r'^orbex\.addons\.\w+\.tests')


def run_tests(env, test_tags, modules=None, reload_tests=False):
    """Run tests for the given modules and test tags."""

    if orbex.cli.COMMAND != 'shell':
        _logger.error('run_tests should be used only in orbex shell')
        return

    if orbex.tools.config['workers'] != 0:
        _logger.error('run_tests should be used only in threaded mode')
        return

    from orbex.service.server import server  # noqa: PLC0415
    if not server.httpd:
    ...
    orbex.tools.config['test_tags'] = test_tags
    orbex.tools.config['test_enable'] = True
    report = _run_tests(env.cr.dbname, modules)
    orbex.tools.config['test_enable'] = None
    orbex.tools.config['test_tags'] = None
