...
def get_test_modules(module):
    ...
    results = _get_tests_modules(importlib.util.find_spec(f'orbex.addons.{module}'))
...
def _get_upgrade_test_modules(module):
    upgrade_modules = (
        f"orbex.upgrade.{module}",
        f"orbex.addons.{module}.migrations",
        f"orbex.addons.{module}.upgrades",
    )
...
def make_suite(module_names, position='at_install'):
    ...
    return orbexSuite(sorted(tests, key=lambda t: getattr(t, 'test_sequence', 0)))
...
def run_suite(suite, global_report=None):
    ...
    results = orbexTestResult(global_report=global_report)
    suite(results)
