...
from orbex.tools.misc import OrderedSet


def transpile_javascript(url, content):
    """
    Transpile the code from native JS modules to custom orbex modules.
    ...
    legacy_orbex_define = get_aliased_orbex_define_content(module_path, content)
    ...
    steps = [
        ...
        partial(wrap_with_orbex_define, module_path, dependencies),
        partial(convert_t, url)
    ]
    for s in steps:
        content = s(content)
    if legacy_orbex_define:
        content += legacy_orbex_define
    return content
...
def url_to_module_path(url):
    """
    orbex modules each have a name. (orbex.define("<the name>", [<dependencies>], function (require) {...});
...
def wrap_with_orbex_define(module_path, dependencies, content):
    ...
    """
    return f"""orbex.define({module_path!r}, {list(dependencies)}, function (require) {{
'use strict';
let __exports = {{}};
{content}
return __exports;
...
def get_aliased_orbex_define_content(module_path, content):
    """
    ...
    > orbex.define("web/chrome/abstract_action", ['web.AbstractAction'], function (require) {
    ...
    .. code-block:: javascript
        ...
        /** @orbex-module */
        // after
        /** @orbex-module alias=web.AbstractAction */
    ...
    .. code-block:: javascript
        ...
        /** @orbex-module */
        // after
        /** @orbex-module alias=web.AbstractAction default=false */
    ...
    if matchobj:
        ...
        if alias:
            if matchobj['default']:
                return """\norbex.define(`%s`, ['%s'], function (require) {
                        return require('%s');
