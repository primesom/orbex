...
STANDARD_MODULES = ['web', 'web_theme', 'theme_common', 'base']
...
class Cloc(object):
    ...
    def count_path(self, path, exclude=None):
        path = path.rstrip('/')
        exclude_list = []
        for i in orbex.modules.module.MANIFEST_NAMES:
    ...
    def count_modules(self, env):
        ...
        exclude_path = {
            ...
            if (m := orbex.modules.Manifest.for_addon(name, display_warning=False))
        }
        ...
        for module_name in module_list:
            manifest = orbex.modules.Manifest.for_addon(module_name)
    ...
    def count_customization(self, env):
        ...
        # the module should be orbex/studio unless it comes from an imported module install
    ...
    def count_database(self, database):
        registry = orbex.modules.registry.Registry(database)
    ...
    def report(self, verbose=False, width=None):
        ...
        # Render
        s = fmt.format(k="orbex cloc", lines="Line", other="Other", code="Code")
        s += hr
