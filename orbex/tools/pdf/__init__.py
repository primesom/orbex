...
from orbex import modules
from orbex.tools.arabic_reshaper import reshape
from orbex.tools.parse_version import parse_version
from orbex.tools.misc import file_open, SENTINEL
...
def add_banner(pdf_stream, text=None, logo=False, thickness=SENTINEL):
    """ Add a banner on a PDF in the upper right corner, with orbex's logo (optionally).

    :param pdf_stream (BytesIO):    The PDF stream where the banner will be applied.
    :param text (str):              The text to be displayed.
    :param logo (bool):             Whether to display orbex's logo in the banner.
    ...
    with file_open('base/static/img/main_partner-image.png', mode='rb') as f:
        orbex_logo_file = io.BytesIO(f.read())
    orbex_logo = Image.open(orbex_logo_file)
    orbex_color = colors.Color(113 / 255, 75 / 255, 103 / 255, 0.8)
    ...
    for p in range(old_pdf.getNumPages()):
        ...
        logo and can.drawImage(
            ImageReader(orbex_logo), 0.25 * thickness, -2.05 * thickness, 40, 40, mask='auto', preserveAspectRatio=True)
...
class orbexPdfFileReader(PdfFileReader):
...
class orbexPdfFileWriter(PdfFileWriter):
    ...
    def embed_orbex_attachment(self, attachment, subtype=None):
        assert attachment, "embed_orbex_attachment cannot be called without attachment."
