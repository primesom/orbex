...
from orbex.tests.common import tagged, new_test_user, JsonRpcException, TransactionCase
from orbex.tools import mute_logger

from orbex.addons.base.tests.common import HttpCase
from orbex.addons.crm.tests.common import TestCrmCommon
from orbex.addons.mail.controllers.thread import ThreadController
...
from orbex.addons.website_crm_partner_assign.controllers.main import (
    WebsiteAccount,
...
class TestPartnerAssign(TransactionCase):
    ...
    def setUp(self):
        ...
        patcher = patch('orbex.addons.base_geolocalize.models.base_geocoder.BaseGeocoder.geo_find', wraps=geo_find)
...
class TestPartnerLeadPortal(TestCrmCommon, HttpCase):
    ...
    def test_portal_post_child_contact_assigned(self):
        ...
        with self.assertRaises(JsonRpcException), mute_logger('orbex.http'):
            self.make_jsonrpc_request(
                route="/mail/message/post",
                params={
                    'thread_model': self.lead_portal._name,
    ...
    @patch('orbex.http.GeoIP')
    def test_03_crm_partner_assign_geolocalization(self, GeoIpMock):
        """
            This test checks situation when "{OrbexURL}/partners" is visited from foreign country without resellers.
        ...
        """
        # Patch GeoIp so it acts, as if Orbex client is located in Mexico
