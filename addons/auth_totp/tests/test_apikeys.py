...
from orbex.tests import HttpCase, get_db_name, tagged
...
class TestAPIKeys(TestTOTPMixin, HttpCase):
    ...
    def test_addremove(self):
        db = get_db_name()
        self.start_tour('/orbex', 'apikeys_tour_setup', login=self.user_test.login)
        ...
        self.start_tour('/orbex', 'apikeys_tour_teardown', login=self.user_test.login)

    def test_apikeys_totp(self):
        db = get_db_name()
        self.install_totphook()
        self.start_tour('/orbex', 'apikeys_tour_setup', login=self.user_test.login)
        self.start_tour('/orbex', 'totp_tour_setup', login=self.user_test.login)
