...
test("ORBEX.ACCOUNT.GROUP type", async function () {
    const { store: composer } = await makeStore(CellComposerStore);

    composer.startEdition("=ORBEX.ACCOUNT.GROUP(");
    ...
    await animationFrame();
    expect(composer.currentContent).toBe('=ORBEX.ACCOUNT.GROUP("asset_receivable"');
