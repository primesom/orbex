...
featurePluginRegistry.add("orbexAccountingAggregates", AccountingPlugin);
...
cellMenuRegistry.add("move_lines_see_records", {
    ...
    async execute(env, newWindow) {
        ...
        if ( func.functionName === "ORBEX.PARTNER.BALANCE" ) {
            [partner_ids, codes, date_range, offset, companyId, includeUnposted] = parsed_args;
        } else if ( func.functionName === "ORBEX.BALANCE.TAG" ) {
        ...
        if ( date_range?.value && !isEvaluationError(date_range.value) ) {
            ...
            if ( ["ORBEX.PARTNER.BALANCE", "ORBEX.RESIDUAL", "ORBEX.BALANCE.TAG"].includes(func.functionName) ) {
        ...
        if ( func.functionName === "ORBEX.BALANCE.TAG" ) {
        ...
        } else if ( func.functionName === "ORBEX.PARTNER.BALANCE" ) {
