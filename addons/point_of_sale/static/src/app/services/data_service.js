...
import { markRaw, reactive } from "@orbex/owl";
...
export class PosData {
    ...
    async checkConnectivity() {
        try {
            ...
            // if the Orbex server is still accessible.
            //
            // This method also makes it possible to run local tests when no connection is
            // available and an Orbex server is running locally.
            //
    ...
    initializeWebsocket() {
        this.onNotified = getOnNotified(this.bus, orbex.access_token);
    }
    ...
    get databaseName() {
        return `point-of-sale-${orbex.pos_config_id}-${orbex.info?.db}`;
    }
        ...
        if (
            ...
            try {
                ...
                const data = await this.orm.call(
                    "pos.session",
                    "load_data",
                    [orbex.pos_session_id, PosData.modelToLoad],
                    {
                        context: {
    ...
    async loadFieldsAndRelations() {
        const key = `pos_data_params_${orbex.pos_config_id}`;
            ...
            try {
                if (["product.product", "product.template"].includes(model)) {
                    ...
                    await this.callRelated(
                        ...
                        [orbex.pos_config_id, [[domain, "in", Array.from(ids)]], 0, 0],
                        {
                            context: {
                                load_archived: true,
                            },
    ...
    async loadServerOrders(domain) {
        ...
        const config = this.models["pos.config"].get(orbex.pos_config_id);
        const session = this.models["pos.session"].get(orbex.pos_session_id);
