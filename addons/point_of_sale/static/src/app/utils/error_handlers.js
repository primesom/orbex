...
import { orbexExceptionTitleMap, ErrorDialog } from "@web/core/errors/error_dialogs";
...
export function handleRPCError(error, dialog) {
    const { data } = error;
    if (orbexExceptionTitleMap.has(error.exceptionName)) {
        const title = orbexExceptionTitleMap.get(error.exceptionName).toString();
    ...
    if (originalError instanceof ConnectionLostError) {
        ...
        if (!network.warningTriggered) {
            env.services.dialog.add(AlertDialog, {
                ...
                body: _t(
                    "Until the connection is reestablished, Orbex Point of Sale will operate with limited functionality."
                ),
