...
from orbex import exceptions
from orbex.addons.mail.models.mail_message import MailMessage
from orbex.addons.mail.models.mail_thread import MailThread
from orbex.addons.mail.tests.common import mail_new_test_user, MailCommon
from orbex.addons.test_mail.data import test_mail_data
from orbex.addons.test_mail.data.test_mail_data import MAIL_TEMPLATE, MAIL_TEMPLATE_EXTRA_HTML, THAI_EMAIL_WINDOWS_874
from orbex.addons.test_mail.models.mail_test_ticket import MailTestTicket
from orbex.addons.test_mail.models.test_mail_models import MailTestGateway, MailTestGatewayGroups
from orbex.sql_db import Cursor
from orbex.tests import Form, tagged, RecordCapturer
from orbex.tools import mute_logger
from orbex.tools.mail import email_normalize, email_split_and_format, formataddr
...
class TestEmailParsing(MailCommon):
    ...
    def test_message_parse_and_replace_bad_content_type(self):
        ...
        for content_type in ('binary/octet-stream', '*/*', 'bin/plain'):
            with self.subTest(content_type=content_type):
                ...
                with self.assertLogs("orbex.addons.mail.models.mail_thread", level="WARNING") as capture:
                ...
                self.assertEqual(capture.output, [
                    ("WARNING:orbex.addons.mail.models.mail_thread:Message containing an unexpected "
    ...
    def test_message_parse_eml(self):
        ...
        mail = self.format(test_mail_data.MAIL_EML_ATTACHMENT, email_from='"Sylvie Lelitre" <test.sylvie.lelitre@agrolait.com>', to=f'generic@{self.alias_domain}',
                           msg_id='<cb7eaf62-58dc-2017-148c-305d0c78892f@orbexsuite.com>',
                           references='<f3b9f8f8-28fa-2543-cab2-7aa68f679ebb@orbexsuite.com>',
    ...
    def test_message_parse_extract_bounce_rfc822_headers_qp(self):
        ...
        incoming_bounce = self.format(
            ...
            email_from='MAILER-DAEMON@mailserver.orbexsuite.com (Mail Delivery System)',
            email_to='bounce@xxx.orbexsuite.com',
            delivered_to='bounce@xxx.orbexsuite.com'
        )
...
class TestMailgateway(MailGatewayCommon):
    ...
    @mute_logger('orbex.addons.mail.models.mail_thread', 'orbex.models')
    ...
    @mute_logger('orbex.addons.mail.models.mail_thread')
    ...
    def test_message_process_followers(self):
        """ Incoming email: recognized author not archived and not orbexbot:
        ...
        orbexbot = self.env.ref('base.partner_root')
        orbexbot.active = True
        orbexbot.email = 'orbexbot@example.com'
        with self.mock_mail_gateway():
            record4 = self.format_and_process(
                MAIL_TEMPLATE, orbexbot.email_formatted, f'groups@{self.alias_domain}',
                subject='Orbexbot Automatic Answer')

        self.assertEqual(record4.message_ids[0].author_id, orbexbot)
        self.assertEqual(record4.message_ids[0].email_from, orbexbot.email_formatted)
        self.assertEqual(record4.message_follower_ids.partner_id, self.env['res.partner'],
                         'message_process: orbexbot -> no follower')
    ...
    @mute_logger('orbex.addons.mail.models.mail_mail', 'orbex.addons.mail.models.mail_thread', 'orbex.models.unlink', 'orbex.tests')
    ...
    @mute_logger('orbex.addons.mail.models.mail_mail', 'orbex.addons.mail.models.mail_thread', 'orbex.models')
    def test_message_route_alias_owner_author_notify(self):
        ...
        self.assertFalse(self.user_root.active, 'notification logic relies on orbexbot being archived')
        ...
        for email_to, test_user in itertools.product(email_tos, test_users):
            with self.subTest(test_user=test_user, email_to=email_to):
                ...
                self.assertEqual(new_messages.create_uid, self.user_root,
                                 'Orbexbot should be creating the message')
    ...
    @mute_logger('orbex.addons.mail.models.mail_thread', 'orbex.models.unlink', 'orbex.addons.mail.models.mail_mail')
    ...
    @mute_logger('orbex.addons.mail.models.mail_thread', 'orbex.addons.mail.models.mail_mail', 'orbex.models.unlink')
    ...
    @mute_logger('orbex.addons.mail.models.mail_thread', 'orbex.addons.mail.models.mail_mail', 'orbex.models', 'orbex.sql_db')
    def test_message_process_alias_config_invalid_defaults(self):
        ...
        with self.mock_mail_gateway(), patch('orbex.addons.mail.models.mail_alias.MailAlias._alias_bounce_incoming_email',
        ...
        with self.assertRaises(Exception), patch('orbex.addons.mail.models.mail_alias.MailAlias._alias_bounce_incoming_email',
                                                 autospec=True) as _alias_bounce_incoming_email_mock:
    ...
    @mute_logger('orbex.addons.mail.models.mail_thread', 'orbex.models', 'orbex.tests')
    ...
    @mute_logger('orbex.addons.mail.models.mail_thread', 'orbex.models.unlink')
    ...
    @mute_logger('orbex.addons.mail.models.mail_thread', 'orbex.models.unlink', 'orbex.addons.mail.models.mail_mail', 'orbex.tests')
    def test_message_process_external_notification_reply(self):
        ...
        self.assertFalse(new_msg.is_internal, "Responses to messages sent by orbexbot should always be public.")
        ...
        new_msg = self.test_record.message_ids[0]
        self.assertTrue(new_msg.is_internal, "Responses to messages sent by anyone but orbexbot should keep"
    ...
    def test_message_process_references_external(self):
        """ Incoming email being a reply to an external email processed by orbex should update thread accordingly """
    ...
    def test_message_process_references_external_buggy_message_id(self):
        ...
        orbex should update thread accordingly. Special case when the
    ...
    def test_message_process_extra_model_res_id(self):
        ...
        # Orbex 10 update: compat mode has been removed and should not work anymore
    ...
    def test_message_hebrew_iso8859_8_i(self):
        ...
        # python, check that Orbex is still capable of decoding it.
...
class TestMailGatewayLoops(MailGatewayCommon):
    ...
    @mute_logger('orbex.addons.mail.models.mail_mail', 'orbex.addons.mail.models.mail_thread')
    ...
    def test_routing_loop_auto_notif(self):
        """ Test Orbex servers talking to each other """
        with self.mock_mail_gateway():
            record = self.format_and_process(
                MAIL_TEMPLATE,
                self.other_partner.email_formatted,
    ...
    def test_routing_loop_follower_alias(self):
        ...
        # simulate this email coming back to the same Orbex server -> msg_id is
...
class TestMailGatewayReplies(MailGatewayCommon):
        ...
        self.assertMailNotifications(
            ...
            [{
                ...
                'email_values': {
                    ...
                    'references': f'{email.message_id} {log.message_id} {reply.message_id}',  # should contain reference to OrbexExternal message, logs to fill up history
                },
                'mail_mail_values': {
    ...
    def test_routing_reply_internal_messages(self):
        """ Test routing notably between two Orbexs when internal messages
        ...
        Action                  Orbex1                   Orbex2
        RFQ-like                                        creation log
                                                        initial_msg
        Orbex2 replies           creation log
                                reply                   reply
        -some internal work-                            user_notification
        Orbex1 replies           reply_2                 reply_2 (incoming email)
        -some internal work-                            log
        Orbex2 replies           reply_3                 reply_3 (outgoing email)

        Purpose: have references from Orbex2 containing message IDs to try to
        ...
        gateway_record.message_post(
            author_id=self.env.ref('base.partner_root').id,
            body='OrbexExternal Inquiry',
            email_from=self.partner_1.email_normalized,
            message_type='comment',
            subtype_id=self.env.ref('mail.mt_comment').id,
        )
        self.assertEqual(gateway_record.message_partner_ids, self.partner_admin)
        log, orbexext_msg = gateway_record.message_ids[1], gateway_record.message_ids[0]
        self.assertEqual(orbexext_msg.parent_id, log, 'Log serves as thread ancestor')
        ...
        self.assertMailNotifications(
            ...
            [{
                ...
                'email_values': {
                    'message_id': reply.message_id,
                    'references': f'{log.message_id} {orbexext_msg.message_id} {reply.message_id}',  # should contain reference to OrbexExternal message
                },
                'mail_mail_values': {
                    'notified_partner_ids': self.partner_1 + self.partner_admin,
                    'parent_id': orbexext_msg,  # attached to last comment / email when possible
                },
                'notif': [
                    {'partner': self.partner_1, 'type': 'email',},
        ...
        # coming from Orbex1: their reply as an incoming email
        with self.mock_mail_gateway():
            self.format_and_process(
                MAIL_TEMPLATE, self.email_from, reply.reply_to,
                subject='Gateway Creation',
                date=datetime.now(),
                extra=f'References: {reply.message_id} <msg1@orbex1>',
                debug_log=True,
            )
        ...
        self.assertMailNotifications(
            ...
            [{
                ...
                'email_values': {
                    ...
                    'references': f'{log.message_id} {orbexext_msg.message_id} {reply.message_id} {reply_2.message_id}',  # should contain reference to OrbexExternal message
                },
                'mail_mail_values': {
        ...
        self.assertMailNotifications(
            reply_3,
            [{
                'content': 'Orbex Reply 2',
                'email_values': {
                    'message_id': reply_3.message_id,
                    'references': f'{orbexext_msg.message_id} {reply.message_id} {reply_2.message_id} {reply_3.message_id}',  # should contain reference to OrbexExternal message
                },
                'mail_mail_values': {
                    'notified_partner_ids': self.partner_1 + self.partner_admin,
