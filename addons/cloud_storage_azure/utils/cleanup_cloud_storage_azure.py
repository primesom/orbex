...
# orbex
orbex_url = 'http://localhost:8069'
orbex_db = 'orbex_db'
orbex_username = 'admin'
orbex_password = 'admin'
...
def get_blobs_to_be_deleted(blob_urls, batch_size=1000):
    common = xmlrpc.client.ServerProxy(f'{orbex_url}/xmlrpc/2/common')
    uid = common.authenticate(orbex_db, orbex_username, orbex_password, {})
    models = xmlrpc.client.ServerProxy(f'{orbex_url}/xmlrpc/2/object')
    for blob_urls_ in split_every(batch_size, blob_urls):
        blob_urls_ = list(blob_urls_)
        attachments = models.execute_kw(orbex_db, uid, orbex_password, 'ir.attachment', 'search_read', [
