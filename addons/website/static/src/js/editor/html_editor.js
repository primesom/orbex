    ...
    isFrontendUrl(url) {
        ...
        return (
            (browser.location.hostname === parsedUrl.hostname ||
                // Also check if the orbex-hosted domain is the current domain of the url
                new RegExp(`^https?://${session.db}\\.orbex\\.com(/.*)?$`).test(parsedUrl.origin)) &&
            !parsedUrl.pathname.startsWith("/orbex") &&
