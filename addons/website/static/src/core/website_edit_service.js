...
patch(Colibri.prototype, {
    protectSyncAfterAsync(interaction, name, fn) {
        ...
        return (...args) => {
            ...
            wysiwyg?.orbexEditor.observerUnactive(fullName);
            const result = fn(...args);
            wysiwyg?.orbexEditor.observerActive(fullName);
            return result;
        };
    },
    addListener(target, event, fn, options) {
        ...
        const wysiwyg = window.$?.("#wrapwrap").data("wysiwyg");
        let stealthFn = fn;
        if (wysiwyg?.orbexEditor && !fn.isHandler && stealth) {
            const name = `${this.interaction.constructor.name}/${event}`;
            stealthFn = (...args) => {
                wysiwyg.orbexEditor.observerUnactive(name);
                const result = fn(...args);
                wysiwyg.orbexEditor.observerActive(name);
                return result;
            };
        }
        return super.addListener(target, event, stealthFn, options);
