...
class TestWebsiteAssets(orbex.tests.HttpCase):
    ...
    def test_01_multi_domain_assets_generation(self):
        ...
        self.url_open(domain_1 + '/orbex')
        check_asset()
        self.url_open(domain_2 + '/orbex')
        check_asset()
        self.url_open(domain_1 + '/orbex')
        check_asset()
        self.url_open(domain_2 + '/orbex')
...
class TestWebAssets(orbex.tests.HttpCase):
    def test_assets_url_validation(self):
        website_id = self.env['website'].search([], limit=1, order='id desc').id

        with orbex.tools.mute_logger('orbex.addons.web.controllers.binary'):
            self.assertEqual(
