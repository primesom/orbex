...
from orbex import http, models, fields, tools, _
from orbex.exceptions import AccessError, UserError
from orbex.fields import Domain
from orbex.http import request, SessionExpiredException
from orbex.tools import OrderedSet, escape_psql, html_escape as escape, py_to_js_locale
from orbex.tools.translate import LazyTranslate
from orbex.addons.base.models.ir_http import EXTENSION_TO_WEB_MIMETYPES
from orbex.addons.portal.controllers.portal import pager as portal_pager
from orbex.addons.portal.controllers.web import Home
from orbex.addons.web.controllers.binary import Binary
from orbex.addons.web.controllers.session import Session
from orbex.addons.website.tools import get_base_domain
from orbex.tools.json import scriptsafe as json
from orbex.tools.translate import TRANSLATED_ELEMENTS
...
class Website(Home):
    ...
    def _login_redirect(self, uid, redirect=None):
        ...
        if not redirect and request.params.get('login_success'):
            if request.env['res.users'].browse(uid)._is_internal():
                redirect = '/orbex?' + request.httprequest.query_string.decode()
            else:
                redirect = '/my'
    ...
    def website_info(self, **kwargs):
        ...
        values = {
            'apps': apps,
            'l10n': l10n,
            'version': orbex.service.common.exp_version()
        }
    ...
    def website_configurator(self, step=1, **kwargs):
        ...
        action_url = f"/orbex/action-website.website_configurator?menu_id={request.env.ref('website.menu_website_configuration').id}"
        if step > 1:
    ...
    def pagenew(self, path="", add_menu=False, template=False, redirect=False, **kwargs):
        ...
        if redirect:
            if ext_special_case:  # redirect non html pages to backend to edit
                return request.redirect(f"/orbex/ir.ui.view/{page.get('view_id')}")
