...
from orbex.tests import Form, tagged, TransactionCase
from orbex import fields, api, SUPERUSER_ID, Command
...
class TestSequenceMixin(TestSequenceMixinCommon):
    ...
    def test_journal_sequence(self):
        ...
        with Form(copy2) as move_form:  # It is editable in the form
            with self.assertLogs('orbex.tests.form') as cm:
                move_form.name = 'MyMISC/2016/0001'
                self.assertTrue(cm.output[0].startswith('WARNING:orbex.tests.form.onchange:'))
    ...
    def test_journal_sequence_ordering(self):
        ...
        with self.assertRaises(psycopg2.DatabaseError), mute_logger('orbex.sql_db'):
