...
from orbex.exceptions import AccessDenied, AccessError, UserError
from orbex.fields import Domain
from orbex.http import request
from orbex.modules.module import MANIFEST_NAMES, Manifest
from orbex.release import major_version
from orbex.tools import SQL, convert_file
from orbex.tools import file_open, file_path, file_open_temporary_directory, ormcache
from orbex.tools.misc import OrderedSet, topological_sort
from orbex.tools.translate import JAVASCRIPT_TRANSLATION_COMMENT, CodeTranslations, TranslationImporter, get_base_langs

from orbex.addons.base.models.ir_asset import is_wildcard_glob

_logger = logging.getLogger(__name__)

APPS_URL = "https://apps.orbexsuite.com"
...
class IrModuleModule(models.Model):
    ...
    def _import_module(self, module, path, force=False, with_demo=False):
        ...
        if unmet_dependencies:
            ...
            raise UserError(_("Studio customizations require the Orbex Studio app."))
    ...
    def _extract_resource_attachment_translations(self, module, lang):
        ...
        for attachment in attachments.filtered('raw'):
            ...
            if attachment.url.endswith('js'):
                extract_method = 'orbex.tools.babel:extract_javascript'
                extract_keywords = {'_t': None}
            else:
                extract_method = 'orbex.tools.translate:babel_extract_qweb'
                extract_keywords = {}
            try:
                with io.BytesIO(attachment.raw) as fileobj:
