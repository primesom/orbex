...
from orbex import api, fields, models, tools, SUPERUSER_ID, _
from orbex.fields import Command, Date, Domain
from orbex.addons.rating.models import rating_data
from orbex.addons.html_editor.tools import handle_history_divergence
from orbex.exceptions import UserError, ValidationError
from orbex.tools import format_list, SQL, LazyTranslate, html_sanitize
from orbex.addons.resource.models.utils import filter_domain_leaf
from orbex.addons.project.controllers.project_sharing_chatter import ProjectSharingChatter
...
class ProjectTask(models.Model):
    ...
    # Need this field to check there is no email loops when Orbex reply automatically
    ...
    def _notify_by_email_get_headers(self, headers=None):
        ...
        if self.project_id:
            current_objects = [h for h in headers.get('X-Orbex-Objects', '').split(',') if h]
            current_objects.insert(0, 'project.project-%s, ' % self.project_id.id)
            headers['X-Orbex-Objects'] = ','.join(current_objects)
        if self.tag_ids:
            headers['X-Orbex-Tags'] = ','.join(self.tag_ids.mapped('name'))
        return headers
    ...
    def action_redirect_to_project_task_form(self):
        ...
        return {
            'type': 'ir.actions.act_url',
            'url': f"/orbex/{self.project_id.id}/action-project.act_project_project_2_project_task_all/{self.id}?menu_id={menu_id}",
            'target': 'new',
        }
