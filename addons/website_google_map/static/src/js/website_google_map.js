...
function initialize_map() {
    ...
    var map = new google.maps.Map(document.getElementById('orbex-google-map'), {
        zoom: 1,
    ...
    async function initializeMarkers(orbexPartnerData) {
        // Create the markers and cluster them on the map
        if (orbexPartnerData) { /* orbex_partner_data special variable should have been defined in google_map.xml */
            const markerPromises = [];
            for (var i = 0; i < orbex_partner_data.counter; i++) {
                const prom = set_marker(orbex_partner_data.partners[i])
    ...
    initializeMarkers(orbex_partner_data);
