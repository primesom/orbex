import { EventBus } from "@orbex/owl";
...
export const presenceService = {
    start(env) {
        ...
        function onFocusChange(isFocused) {
            ...
            isOrbexFocused = isFocused;
            browser.localStorage.setItem(`${LOCAL_STORAGE_PREFIX}.focus`, isOrbexFocused);
            if (isOrbexFocused) {
                lastPresenceTime = luxon.DateTime.now().ts;
                env.bus.trigger("window_focus", isOrbexFocused);
            }
        }

        function onStorage({ key, newValue }) {
            if (key === `${LOCAL_STORAGE_PREFIX}.focus`) {
                isOrbexFocused = JSON.parse(newValue);
