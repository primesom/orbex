...
import logging
import requests
from orbex.addons.microsoft_calendar.models.microsoft_sync import microsoft_calendar_token
from datetime import datetime, timedelta

from orbex import api, fields, models, _, Command
from orbex.exceptions import UserError
from orbex.loglevels import exception_to_unicode
from orbex.addons.microsoft_account.models import microsoft_service
from orbex.addons.microsoft_calendar.utils.microsoft_calendar import InvalidSyncToken
...
class ResUsers(models.Model):
    ...
    def _sync_microsoft_calendar(self):
        ...
        # Microsoft -> Orbex
        synced_events, synced_recurrences = self.env['calendar.event']._sync_microsoft2orbex(events) if events else (self.env['calendar.event'], self.env['calendar.recurrence'])

        # Orbex -> Microsoft
        recurrences = self.env['calendar.recurrence']._get_microsoft_records_to_sync(full_sync=full_sync)
        recurrences -= synced_recurrences
        recurrences._sync_orbex2microsoft()
        ...
        (events - synced_events)._sync_orbex2microsoft()
    ...
    def _set_ICP_first_synchronization_date(self, now):
        ...
        created Orbex events and thus avoid spamming invitations for those events.
        """
