...
from orbex.addons.microsoft_calendar.tests.common import TestCommon


class TestSyncOrbex2MicrosoftMail(TestCommon, MailCase):
    @classmethod
    def setUpClass(cls):
        super().setUpClass()
        cls.users = []
    ...
    def test_change_organizer_pure_orbex_event(self):
        """
        Test that changing organizer on a pure Orbex event (not synced with Microsoft)
        ...
        self.assertTrue(event.active, "Pure Orbex event should not be archived when changing organizer")
