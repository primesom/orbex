...
class TestCommon(HttpCase):
    ...
    def setUp(self):
        ...
        m = mute_logger('orbex.addons.auth_signup.models.res_users')
        ...
        # simple event values to create a Orbex event
        ...
        # -----------------------------------------------------------------------------------------
        # Expected values for Orbex events converted to Outlook events (to be posted through API)
        # -----------------------------------------------------------------------------------------
        ...
        # -----------------------------------------------------------------------------------------
        # Expected values for Outlook events converted to Orbex events
        # -----------------------------------------------------------------------------------------

        self.expected_orbex_event_from_outlook = {
            "name": "simple_event",
        ...
        self.expected_orbex_recurrency_from_outlook = {
            'active': True,
            'byday': '1',
            'count': 0,
            'day': 0,
        ...
        self.expected_orbex_recurrency_events_from_outlook = [
            {
    ...
    def sync_orbex_recurrences_with_outlook_feature(self):
        """
    ...
    def create_events_for_tests(self):
        ...
        # Currently, it is forbidden to create recurrences in Orbex. A trick for deactivating the checking
    ...
    def assert_orbex_event(self, orbex_event, expected_values):
        """
        Assert that an Orbex event has the same values than in the expected_values dictionary,
        for the keys present in expected_values.
        """
        self.assertTrue(expected_values)

        orbex_event_values = orbex_event.read(list(expected_values.keys()))[0]
        for k, v in expected_values.items():
            if k in ("user_id", "recurrence_id"):
                v = (v.id, v.name) if v else False

            if isinstance(v, list):
                self.assertListEqual(sorted(v), sorted(orbex_event_values.get(k)), msg=f"'{k}' mismatch")
            else:
                self.assertEqual(v, orbex_event_values.get(k), msg=f"'{k}' mismatch")

    def assert_orbex_recurrence(self, orbex_recurrence, expected_values):
        """
        Assert that an Orbex recurrence has the same values than in the expected_values dictionary,
        for the keys present in expected_values.
        """
        orbex_recurrence_values = orbex_recurrence.read(list(expected_values.keys()))[0]

        for k, v in expected_values.items():
            self.assertEqual(v, orbex_recurrence_values.get(k), msg=f"'{k}' mismatch")
