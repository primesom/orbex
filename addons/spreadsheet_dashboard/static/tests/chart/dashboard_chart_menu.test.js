import { beforeEach, expect, test } from "@orbex/hoot";
...
test("Click on chart in dashboard mode redirect to the orbex menu", async function () {
    ...
    await click(fixture.querySelector(".o-chart-container canvas"));
    await animationFrame();
    // Clicking on a chart while on dashboard mode redirect to the orbex menu
...
test("Click on chart element in dashboard mode do not redirect twice", async function () {
    ...
    const { model } = await createModelWithDataSource({ serverData });
    const fixture = await mountSpreadsheet(model);
    const chartId = insertChartInSpreadsheet(model, "orbex_pie");
    model.dispatch("LINK_ORBEX_MENU_TO_CHART", { chartId, orbexMenuId: 2 });
    await animationFrame();
    model.updateMode("dashboard");
    await animationFrame();
...
test("Clicking on a scorecard or gauge redirects to the linked menu id", async function () {
    ...
    model.dispatch("LINK_ORBEX_MENU_TO_CHART", { chartId: "scorecardId", orbexMenuId: 2 });
    model.dispatch("LINK_ORBEX_MENU_TO_CHART", { chartId: "gaugeId", orbexMenuId: 2 });
    await animationFrame();
...
test.tags("desktop");
test("Middle-click on chart in dashboard mode open the orbex menu in a new tab", async function () {
    const { model } = await createModelWithDataSource({ serverData });
    await mountSpreadsheet(model);
