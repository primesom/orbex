...
(function (orbex) {
    ...
    class ModuleLoader {
        /** @type {OrbexModuleLoader["bus"]} */
        bus = new EventTarget();
        /** @type {OrbexModuleLoader["checkErrorProm"]} */
        checkErrorProm = null;
        /** @type {OrbexModuleLoader["factories"]} */
        factories = new Map();
        /** @type {OrbexModuleLoader["failed"]} */
        failed = new Set();
        /** @type {OrbexModuleLoader["jobs"]} */
        jobs = new Set();
        /** @type {OrbexModuleLoader["modules"]} */
        modules = new Map();
        ...
        /** @type {OrbexModuleLoader["addJob"]} */
        addJob(name) {
            this.jobs.add(name);
            this.startModules();
        }

        /** @type {OrbexModuleLoader["define"]} */
        define(name, deps, factory, lazy = false) {
            ...
            this.factories.set(name, {
                deps,
                fn: factory,
                ignoreMissingDeps: globalThis.__orbexIgnoreMissingDependencies,
            });
            if (!lazy) {
                this.addJob(name);
        ...
        /** @type {OrbexModuleLoader["findErrors"]} */
        ...
        /** @type {OrbexModuleLoader["findJob"]} */
        findJob() {
        ...
        /** @type {OrbexModuleLoader["reportErrors"]} */
        ...
        /** @type {OrbexModuleLoader["startModules"]} */
        startModules() {
            let job;
        ...
        /** @type {OrbexModuleLoader["startModule"]} */
        startModule(name) {
            /** @type {(dependency: string) => OrbexModule} */
            const require = (dependency) => this.modules.get(dependency);
            this.jobs.delete(name);
            const factory = this.factories.get(name);
            /** @type {OrbexModule | null} */
            let module = null;
            try {
    ...
    orbex.define = loader.define.bind(loader);
    orbex.loader = loader;

    if (orbex.debug && !loader.debug) {
