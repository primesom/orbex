...
/**
 * @typedef {import("../../env").OrbexEnv} OrbexEnv
...
export function swallowAllVisitorErrors(env, error, originalError) {
    if (!user.isInternalUser && !orbex.debug && !session.test_mode) {
        return true;
    }
}
...
if (user.isInternalUser === undefined) {
    ...
    // apparently not be present in all Orbex screens at the moment... TODO ?
