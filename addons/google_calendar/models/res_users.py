...
from orbex import api, fields, models, Command
from orbex.addons.google_calendar.utils.google_calendar import GoogleCalendarService, InvalidSyncToken
from orbex.addons.google_calendar.models.google_sync import google_calendar_token
from orbex.addons.google_account.models import google_service
from orbex.exceptions import LockError
from orbex.loglevels import exception_to_unicode
from orbex.tools import str2bool
...
class ResUsers(models.Model):
    ...
    def _check_pending_orbex_records(self):
    ...
    def _sync_google_calendar(self, calendar_service: GoogleCalendarService):
        ...
        if not events and not self._check_pending_orbex_records():
            return False
        # Google -> Orbex
        ...
        # We apply Google updates only if their write date is later than the write date in Orbex.
        # It's possible that multiple updates affect the same record, maybe not directly.
        # To handle this, we preserve the write dates in Orbex before applying any updates,
        # and use these dates instead of the current live dates.
        orbex_events = self.env['calendar.event'].browse((events - recurrences).orbex_ids(self.env))
        orbex_recurrences = self.env['calendar.recurrence'].browse(recurrences.orbex_ids(self.env))
        recurrences_write_dates = {r.id: r.write_date for r in orbex_recurrences}
        events_write_dates = {e.id: e.write_date for e in orbex_events}
        synced_recurrences = self.env['calendar.recurrence']._sync_google2orbex(recurrences, recurrences_write_dates)
        synced_events = self.env['calendar.event']._sync_google2orbex(events - recurrences, events_write_dates, default_reminders=default_reminders)

        # Orbex -> Google
        recurrences = self.env['calendar.recurrence']._get_records_to_sync(full_sync=full_sync)
        recurrences -= synced_recurrences
        recurrences.with_context(send_updates=send_updates)._sync_orbex2google(calendar_service)
        ...
        (events - synced_events).with_context(send_updates=send_updates)._sync_orbex2google(calendar_service)
    ...
    def _sync_google_calendar_filter_remote_events(self, google_events):
        """Filter out events coming from google which should not be synced into orbex."""
    ...
    def _sync_single_event(self, calendar_service: GoogleCalendarService, orbex_event, event_id):
        self.ensure_one()
        ...
        synced_events = self.env['calendar.event']._sync_google2orbex(event, default_reminders=default_reminders)
        # Orbex -> Google
        orbex_event.with_context(send_updates=send_updates)._sync_orbex2google(calendar_service)
        return bool(orbex_event | synced_events)
