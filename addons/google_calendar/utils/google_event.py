...
class GoogleEvent(abc.Set):
    ...
    Inspired by Orbex recordset, one instance can be a single Google event or a
    ...
    def orbex_id(self, env):
        self.orbex_ids(env)  # load ids
        return self._orbex_id

    def _meta_orbex_id(self, dbname):
        """Returns the Orbex id stored in the Google Event metadata.
        ...
        o_id = properties.get('%s_orbex_id' % dbname)
        if o_id:
            return int(o_id)

    def orbex_ids(self, env):
        ids = tuple(e._orbex_id for e in self if e._orbex_id)
        if len(ids) == len(self):
            return ids
        model = self._get_model(env)
        found = self._load_orbex_ids_from_db(env, model)
        unsure = self - found
        if unsure:
            unsure._load_orbex_ids_from_metadata(env, model)

        return tuple(e._orbex_id for e in self)

    def _load_orbex_ids_from_metadata(self, env, model):
        unsure_orbex_ids = tuple(e._meta_orbex_id(env.cr.dbname) for e in self)
        orbex_events = model.browse(_id for _id in unsure_orbex_ids if _id)

        # Extended properties are copied when splitting a recurrence Google side.
        # Hence, we may have two Google recurrences linked to the same Orbex id.
        # Therefore, we only consider Orbex records without google id when trying
        # to match events.
        o_ids = orbex_events.exists().filtered(lambda e: not e.google_id).ids
        for e in self:
            orbex_id = e._meta_orbex_id(env.cr.dbname)
            if orbex_id in o_ids:
                e._events[e.id]['_orbex_id'] = orbex_id

    def _load_orbex_ids_from_db(self, env, model):
        orbex_events = model.with_context(active_test=False)._from_google_ids(self.ids)
        mapping = {e.google_id: e.id for e in orbex_events}  # {google_id: orbex_id}
        existing_google_ids = orbex_events.mapped('google_id')
        for e in self:
            orbex_id = mapping.get(e.id)
            if orbex_id:
                e._events[e.id]['_orbex_id'] = orbex_id
    ...
    def owner(self, env):
        # Owner/organizer could be desynchronised between Google and Orbex.
        # Let userA, userB be two new users (never synced to Google before).
        # UserA creates an event in Orbex (they are the owner) but userB syncs first.
        ...
        # (they are the organizer in Google). The "real" owner (in Orbex) is stored as an
        ...
        if real_owner_id and real_owner.exists():
            ...
            # In Google: 1 email = 1 user; but in Orbex several users might have the same email :/
    ...
    def clear_type_ambiguity(self, env):
        ...
        recurrences = ambiguous_events._load_orbex_ids_from_db(env, env['calendar.recurrence'])
    ...
    def exists(self, env) -> 'GoogleEvent':
        ...
        recurrences.orbex_ids(env)
        events.orbex_ids(env)

        return self.filter(lambda e: e._orbex_id)
    ...
    def get_orbex_event(self, env):
        if self._get_model(env)._name == 'calendar.event':
            return env['calendar.event'].browse(self.orbex_id(self.env))
        else:
            return env['calendar.recurrence'].browse(self.orbex_id(self.env)).base_event_id
