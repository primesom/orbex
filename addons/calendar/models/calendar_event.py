...
from orbex.addons.base.models.res_partner import _tz_get
from orbex.addons.calendar.models.calendar_attendee import CalendarAttendee
from orbex.addons.calendar.models.calendar_recurrence import (
    weekday_to_field,
...
from orbex.addons.calendar.models.utils import interval_from_events
from orbex.tools.intervals import intervals_overlap
from orbex.tools.translate import _
from orbex.tools.misc import get_lang
from orbex.tools import html2plaintext, html_sanitize, is_html_empty, single_email_re
...
class CalendarEvent(models.Model):
        ...
        for meeting in self:
            ...
            if meeting.alarm_ids:
                for alarm in meeting.alarm_ids:
                    ...
                    trigger.value = delta
                    valarm.add('DESCRIPTION').value = alarm.name or u'Orbex'
    ...
    def _get_contact_details_description(self, organizer, partners):
        ...
        """
        orbexbot = self.env.ref('base.user_root')
        contact_description = []
        # Organizer
        if organizer and organizer != orbexbot:
        ...
        first_partner = partners.filtered(lambda partner: partner not in (orbexbot.partner_id + organizer.partner_id))[:1]
        if first_partner:
