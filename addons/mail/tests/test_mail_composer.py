...
class TestMailComposerForm(TestMailComposer):
    ...
    @mute_logger('orbex.addons.base.models.ir_rule', 'orbex.addons.mail.models.mail_mail')
    @users('employee')
...
class TestMailComposerUI(MailCommon, HttpCase):
    ...
    def test_mail_composer_test_tour(self):
        ...
        with self.mock_mail_app():
            self.start_tour(
                f"/orbex/res.partner/{partner.id}",
    ...
    def test_send_attachment_without_body(self):
        self.start_tour("/orbex/discuss", "create_thread_for_attachment_without_body",login="admin")
