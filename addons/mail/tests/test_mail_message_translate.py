...
class TestTranslationController(HttpCaseWithUserDemo):
    ...
    def test_access_right(self):
        with self.assertRaises(JsonRpcException, msg="orbex.http.SessionExpiredException"):
        ...
        with self.assertRaises(JsonRpcException, msg="orbex.exceptions.AccessError"), mute_logger("orbex.http"):
            self._mock_translation_request({"message_id": self.message.id})
