...
import { Component, onMounted, onWillStart, onWillUnmount, onWillUpdateProps } from "@orbex/owl";
...
export class DiscussClientAction extends Component {
    ...
    async restoreDiscussThread(props) {
        ...
        if (!parsedActiveId) {
            ...
            const orbexbotChat = this.store.orbexbot?.searchChat();
            const selfMember = orbexbotChat?.self_member_id;
            if (orbexbotChat && selfMember?.is_pinned && !selfMember.seen_message_id) {
                orbexbotChat.setAsDiscussThread(false);
            }
            return;
        }
