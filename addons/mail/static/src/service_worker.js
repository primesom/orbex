...
let db;
const unread_store = new Store("orbex-mail-unread-db", "orbex-mail-unread-store");
 ...
 * @param {Object} param1
 * @param {string} [param1.action] orbex client action
...
async function openDiscussChannel(channelId, { action, joinCall = false, source } = {}) {
    ...
    if (action) {
        discussURLRegexes.push(
            ...
            new RegExp(`/orbex/action-${action}`)
        );
    }
    let targetClient;
    ...
    if (action) {
        const url = new URL(`/orbex/action-${action}`, location.origin);
...
self.addEventListener("notificationclick", (event) => {
    ...
    if (event.notification.data) {
        ...
        if (model === "discuss.channel") {
            ...
            event.waitUntil(clients.openWindow(`/orbex/${modelPath}/${res_id}`));
        }
    }
});
