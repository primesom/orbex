...
from orbex import http, tests
from orbex.addons.base.tests.common import HttpCaseWithUserPortal
from orbex.addons.gamification.tests.common import HttpCaseGamification
from orbex.fields import Command, Datetime
...
class TestUi(TestUICommon):
    ...
    @mute_logger("orbex.http", "orbex.addons.base.models.ir_rule", "werkzeug")
    ...
    def test_review_link_redirection(self):
        ...
        cases = [
            ("admin", rf"^/orbex/slide.channel/.*highlight_message_id={message.id}"),
...
class TestUiPublisher(HttpCaseGamification):
    ...
    def fetch_proxy(self, url):
        if url.endswith('ThreeTimeAKCGoldWinnerPembrookeWelshCorgi.jpg'):
            ...
            with file_open('base/tests/orbex.jpg', 'rb') as f:
                content = f.read()
