...
from orbex.addons.iot_drivers.tools import helpers
from orbex.addons.iot_drivers.tools.system import IS_TEST
from orbex.netsvc import ColoredFormatter

_logger = logging.getLogger(__name__)

IOT_LOG_TO_SERVER_CONFIG_NAME = 'iot_log_to_server'  # config name in orbex.conf
...
class AsyncHTTPHandler(logging.Handler):
    ...
    def __init__(self, orbex_server_url, active):
        """
        :param orbex_server_url: Orbex Server URL
        """
        super().__init__()
        self._orbex_server_url = orbex_server_url
    ...
    def toggle_active(self, is_active):
        ...
        if self._active and self._orbex_server_url:
    ...
    def _periodic_flush(self):
        orbex_session = requests.Session()
        while self._orbex_server_url and self._active:  # allow to exit the loop on thread.join
            time.sleep(self._FLUSH_INTERVAL)
            self._flush_logs(orbex_session)

    def _flush_logs(self, orbex_session):
        ...
        if not self._orbex_server_url or queue_size == 0:
            return
        try:
            orbex_session.post(
                self._orbex_server_url + '/iot/log',
                data=empty_queue(),
                headers={'X-Orbex-Database': self._db_name},
...
def get_orbex_config_log_to_server_option():
...
def check_and_update_orbex_config_log_to_server_option(new_state):
    """
    :return: wherever the config file need to be updated or not
    """
    if get_orbex_config_log_to_server_option() != new_state:
...
_server_log_sender_handler = AsyncHTTPHandler(helpers.get_orbex_server_url(), get_orbex_config_log_to_server_option())
if not IS_TEST:
    ...
    # Set it in the 'root' logger, on which every logger (including orbex) is a child
