...
export class TimeDialog extends Component {
    ...
    setup() {
        this.store = useStore();
        this.state = useState({
            orbexUptimeSeconds: this.store.base.orbex_uptime_seconds,
            systemUptimeSeconds: this.store.base.system_uptime_seconds,
        });
        setInterval(() => {
            this.state.orbexUptimeSeconds += 1;
    ...
    startDateFromSeconds(uptimeInSeconds) {
        ...
        const orbexUptimeMs = uptimeInSeconds * 1000;
        return new Date(currentTimeMs - orbexUptimeMs).toUTCString();
    }
        ...
        <BootstrapDialog identifier="'time-dialog'" btnName="'View Uptime'">
            ...
            <t t-set-slot="body">
                <div class="d-flex flex-column gap-4">
                  <div>
                    <h5>Orbex Service</h5>
                    <div>Running for <b t-out="secondsToHumanReadable(state.orbexUptimeSeconds)"/></div>
                    <div class="text-secondary">Started at <b t-out="startDateFromSeconds(state.orbexUptimeSeconds)"/></div>
                  </div>
