...
from orbex.addons.website_hr_recruitment.controllers.main import WebsiteHrRecruitment

@orbex.tests.tagged('post_install', '-at_install')
class TestWebsiteHrRecruitmentForm(orbex.tests.HttpCase):
    def test_tour(self):
        ...
        with orbex.tests.RecordCapturer(self.env['hr.applicant']) as capt:
