...
import { beforeEach, describe, expect, getFixture, test } from "@orbex/hoot";
import { click, queryFirst, waitFor } from "@orbex/hoot-dom";
    ...
    test("Can mount and destroy recursive embedded components in any order", async () => {
        ...
        expect(
            [...fixture.querySelectorAll("[data-embedded]")].filter(
                (elem) => !elem.closest(".orbex-editor-editable")
            )
        ).toEqual([]);
    });
