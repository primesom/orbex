...
import { destroy, expect, getFixture } from "@orbex/hoot";
import { queryOne } from "@orbex/hoot-dom";
import { Component, markup, onWillDestroy, xml } from "@orbex/owl";
...
export async function setupWysiwyg(props = {}) {
    ...
    const el = /** @type {HTMLElement} **/ (
        queryOne(`${props.iframe ? ":iframe " : ""}.orbex-editor-editable`)
    );
    if (content) {
...
export function insertTestHtml(innerHtml) {
    ...
    container.classList.add("orbex-editor-editable");
