...
import { Component, useState, useRef, useEffect, useExternalListener } from "@orbex/owl";
...
export class LinkPopover extends Component {
    ...
    async loadAsyncLinkPreview() {
        ...
        if (!protocol.startsWith("http")) {
            ...
            !new RegExp(`^https?://${session.db}\\.orbex\\.com(/.*)?$`).test(url.origin)
        ) {
    ...
    isAbsoluteURLInCurrentDomain() {
        ...
        // its Orbex instance domain. We just suppose it should be a relative
        // URL (if unexpected behavior, the user can just not enter its Orbex
        // instance domain but its real domain, or opt-out from the domain
        // stripping). Mentioning an .orbexsuite.com domain, especially its own
        // one, is always a bad practice anyway.
        return (
            urlObj.origin === window.location.origin ||
            new RegExp(`^https?://${session.db}\\.orbex\\.com(/.*)?$`).test(urlObj.origin)
        );
    }
}
