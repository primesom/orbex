...
from orbex.addons.auth_signup.models.res_partner import ResPartner
from orbex.addons.mail.tests.common import MailCommon
from orbex.addons.test_mail_full.tests.common import TestMailFullCommon
from orbex.addons.test_mail_sms.tests.common import TestSMSRecipients
from orbex.exceptions import AccessError
from orbex.tests import tagged, users
from orbex.tests.common import HttpCase
from orbex.tools import html_escape, mute_logger
...
class TestPortalControllers(TestPortal):
    ...
    def test_portal_share_comment(self):
        ...
        # Only messages from the current user not OrbexBot
...
class TestPortalFlow(MailCommon, HttpCase):
    ...
    def setUpClass(cls):
        ...
        cls.internal_backend_local_url = f'/orbex/{cls.record_internal._name}/{cls.record_internal.id}'
        cls.portal_backend_local_url = f'/orbex/{cls.record_portal._name}/{cls.record_portal.id}'
        cls.read_backend_local_url = f'/orbex/{cls.record_read._name}/{cls.record_read.id}'
        cls.public_act_url_backend_local_url = f'/orbex/{cls.record_public_act_url._name}/{cls.record_public_act_url.id}'
        cls.discuss_local_url = '/orbex/action-mail.action_discuss'
