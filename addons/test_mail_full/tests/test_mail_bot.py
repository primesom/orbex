...
from orbex.addons.mail.tests.common import MailCommon
from orbex.addons.test_mail.tests.common import TestRecipients
from orbex.tests import tagged
from orbex.tools import mute_logger


@tagged("orbexbot")
class TestOrbexbot(MailCommon, TestRecipients):
    ...
    def setUpClass(cls):
        ...
        cls.orbexbot = cls.env.ref("base.partner_root")
        ...
        cls.orbexbot_ping_body = f'<a href="http://orbexsuite.com/orbex/res.partner/{cls.orbexbot.id}" class="o_mail_redirect" data-oe-id="{cls.orbexbot.id}" data-oe-model="res.partner" target="_blank">@OrbexBot</a>'
        cls.test_record_employe = cls.test_record.with_user(cls.user_employee)

    @mute_logger('orbex.addons.mail.models.mail_mail')
    def test_fetch_listener(self):
        channel = self.user_employee.with_user(self.user_employee)._init_orbexbot()
        orbexbot = self.env.ref("base.partner_root")
        orbexbot_in_fetch_listeners = self.env['discuss.channel.member'].search([('channel_id', '=', channel.id), ('partner_id', '=', orbexbot.id)])
        self.assertEqual(len(orbexbot_in_fetch_listeners), 1, 'orbexbot should appear only once in channel_fetch_listeners')

    @mute_logger('orbex.addons.mail.models.mail_mail')
    def test_orbexbot_ping(self):
        kwargs = self.message_post_default_kwargs.copy()
        kwargs.update({'body': self.orbexbot_ping_body, 'partner_ids': [self.orbexbot.id, self.user_admin.partner_id.id]})
        ...
        with patch('random.choice', lambda x: x[0]):
            self.assertNextMessage(
                ...
                sender=self.orbexbot,
                answer=False
            )
        # Orbexbot should not be a follower but user_employee and user_admin should
        follower = self.test_record.message_follower_ids.mapped('partner_id')
        self.assertNotIn(self.orbexbot, follower)
    ...
    def test_onboarding_flow(self):
        ...
        self.assertNextMessage(
            last_message,  # no message will be post with command help, use last orbexbot message instead
            sender=self.orbexbot,
            answer=("@OrbexBot",)
        )
        kwargs['body'] = ''
        ...
        channel.message_post(**kwargs)
        self.assertNotEqual(self.user_employee.orbexbot_state, 'onboarding_attachement')

        # Test miscellaneous messages
        self.user_employee.orbexbot_state = "idle"
        kwargs['partner_ids'] = []
    ...
    @mute_logger('orbex.addons.mail.models.mail_mail')
    def test_orbexbot_no_default_answer(self):
        kwargs = self.message_post_default_kwargs.copy()
        kwargs.update({'body': "I'm not talking to @orbexbot right now", 'partner_ids': []})
        self.assertNextMessage(
