...
from orbex.tools.view_validation import get_domain_value_names


markdown_link_regex = r"^\[([^\[]+)\]\((.+)\)$"

xml_id_url_prefix = "orbex://ir_menu_xml_id/"

orbex_view_link_prefix = "orbex://view/"


def orbex_charts(data):
    """returns all orbex chart definitions in the spreadsheet"""
    figures = []
    for sheet in data.get("sheets", []):
        for figure in sheet.get("figures", []):
            if figure["tag"] == "chart" and figure["data"]["type"].startswith("orbex_"):
                figures.append(dict(figure["data"], id=figure["id"]))
            elif figure["tag"] == "carousel":
                figures.extend(get_orbex_charts_from_carousel(figure["data"]))
    return figures


def get_orbex_charts_from_carousel(carousel):
    charts = []
    for chart_id, chart in carousel["chartDefinitions"].items():
        if chart["type"].startswith("orbex_"):
...
def orbex_view_links(data):
    """return all view definitions embedded in link cells.
    urls looks like orbex://view/{... view data...}
    """
    return [
        json.loads(url[len(orbex_view_link_prefix):])
        for url in links_urls(data)
        if url.startswith(orbex_view_link_prefix)
    ]
...
def filter_fields(data):
    ...
    charts = orbex_charts(data)
    if "orbexVersion" in data and data["orbexVersion"] < 5:
...
def fields_in_spreadsheet(data):
    ...
    orbex_pivots = (pivot for pivot in data.get("pivots", dict()).values() if pivot.get("type", "ORBEX") == "ORBEX")
    all_fields = chain(
        extract_fields(list_fields, data.get("lists", dict()).values()).items(),
        extract_fields(pivot_fields, orbex_pivots).items(),
        extract_fields(chart_fields, orbex_charts(data)).items(),
        extract_fields(orbex_view_fields, orbex_view_links(data)).items(),
...
def menus_xml_ids_in_spreadsheet(data):
    ...
    return set(data.get("chartOrbexMenusReferences", {}).values()) | {
