...
test("Links between charts and ir.menus are correctly imported/exported", async function () {
    ...
    expect(exportedData.chartOrbexMenusReferences[chartId]).toBe(1, {
        message: "Link to orbex menu is exported",
    });
    const importedModel = new Model(exportedData, { custom: { env } });
    const chartMenu = importedModel.getters.getChartOrbexMenu(chartId);
    expect(chartMenu.id).toBe(1, { message: "Link to orbex menu is imported" });
});

test("Can undo-redo a LINK_ORBEX_MENU_TO_CHART", async function () {
    const env = await makeMockEnv();
    const model = new Model({}, { custom: { env } });
    createBasicChart(model, chartId);
    model.dispatch("LINK_ORBEX_MENU_TO_CHART", {
    ...
    expect(model.getters.getChartOrbexMenu(chartId).id).toBe(1);
    model.dispatch("REQUEST_UNDO");
    expect(model.getters.getChartOrbexMenu(chartId)).toBe(undefined);
...
test("Links of Orbex charts are duplicated when duplicating a sheet", async function () {
    ...
    expect(model.getters.getChartOrbexMenu(newChartId)).toEqual(
        model.getters.getChartOrbexMenu(chartId)
    );
});
