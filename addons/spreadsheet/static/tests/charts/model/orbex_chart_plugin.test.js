...
import { OrbexBarChart } from "@spreadsheet/chart/orbex_chart/orbex_bar_chart";
import { OrbexChart } from "@spreadsheet/chart/orbex_chart/orbex_chart";
import { OrbexLineChart } from "@spreadsheet/chart/orbex_chart/orbex_line_chart";
...
test("Can add an Orbex Bar chart", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "orbex_bar" });
    const sheetId = model.getters.getActiveSheetId();
    expect(model.getters.getChartIds(sheetId).length).toBe(1);
    const chartId = model.getters.getChartIds(sheetId)[0];
    const chart = model.getters.getChart(chartId);
    expect(chart instanceof OrbexBarChart).toBe(true);
...
test("Can add an Orbex Line chart", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "orbex_line" });
    const sheetId = model.getters.getActiveSheetId();
    expect(model.getters.getChartIds(sheetId).length).toBe(1);
    const chartId = model.getters.getChartIds(sheetId)[0];
    const chart = model.getters.getChart(chartId);
    expect(chart instanceof OrbexLineChart).toBe(true);
    expect(chart.getDefinitionForExcel()).toBe(undefined);
    expect(model.getters.getChartRuntime(chartId).chartJsConfig.type).toBe("line");
});

test("Can add an Orbex Pie chart", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "orbex_pie" });
    const sheetId = model.getters.getActiveSheetId();
    expect(model.getters.getChartIds(sheetId).length).toBe(1);
    const chartId = model.getters.getChartIds(sheetId)[0];
    const chart = model.getters.getChart(chartId);
    expect(chart instanceof OrbexChart).toBe(true);
    expect(chart.getDefinitionForExcel()).toBe(undefined);
...
test("Orbex bar chart runtime loads the data", async () => {
    const { model } = await createSpreadsheetWithChart({
        type: "orbex_bar",
...
test("Orbex pie chart runtime loads the data", async () => {
    const { model } = await createSpreadsheetWithChart({
        type: "orbex_pie",
        mockRPC: async function (route, args) {
            if (args.method === "formatted_read_group") {
...
test("Orbex line chart runtime loads the data", async () => {
    const { model } = await createSpreadsheetWithChart({
        type: "orbex_line",
        mockRPC: async function (route, args) {
            if (args.method === "formatted_read_group") {
...
test("Data reloaded upon domain update for charts other than pie/bar/line", async () => {
    ...
    updateChart(model, chartId, { type: "orbex_pie" });
    await waitForDataLoaded(model);
...
test("Can import/export an Orbex chart", async () => {
    const { model } = await createModelWithDataSource();
    insertChartInSpreadsheet(model, "orbex_line");
    const data = model.exportData();
    const figures = data.sheets[0].figures;
    expect(figures.length).toBe(1);
    const figure = figures[0];
    expect(figure.tag).toBe("chart");
    expect(figure.data.type).toBe("orbex_line");
...
test("Can undo/redo an Orbex chart creation", async () => {
    const { model } = await createModelWithDataSource();
    insertChartInSpreadsheet(model, "orbex_line");
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
...
test("charts with no legend", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "orbex_pie" });
    insertChartInSpreadsheet(model, "orbex_bar");
    insertChartInSpreadsheet(model, "orbex_line");
    const sheetId = model.getters.getActiveSheetId();
...
test("Can copy/paste Orbex chart", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "orbex_pie" });
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
    ...
    expect(model.getters.getChart(chartIds[1]) instanceof OrbexChart).toBe(true);
...
test("Can cut/paste Orbex chart", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "orbex_pie" });
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
    ...
    expect(model.getters.getChart(chartIds[0]) instanceof OrbexChart).toBe(true);
    expect(JSON.stringify(model.getters.getChartRuntime(chartIds[0]))).toBe(
        JSON.stringify(chartRuntime)
    );
});

test("Duplicating a sheet correctly duplicates Orbex chart", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "orbex_bar" });
    const sheetId = model.getters.getActiveSheetId();
...
test("Load orbex chart spreadsheet with models that cannot be accessed", async function () {
...
test("Can insert orbex chart from a different model", async () => {
    const { model } = await createModelWithDataSource();
...
test("Orbex chart legend color changes with background color update", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "orbex_bar" });
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
    const definition = model.getters.getChartDefinition(chartId);
...
test("Remove orbex chart when sheet is deleted", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "orbex_line" });
    const sheetId = model.getters.getActiveSheetId();
    ...
    expect(model.getters.getOrbexChartIds().length).toBe(1);
    model.dispatch("DELETE_SHEET", { sheetId });
    expect(model.getters.getOrbexChartIds().length).toBe(0);
});

test("Orbex chart datasource display name has a default when the chart title is empty", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "orbex_line" });
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
    const definition = model.getters.getChartDefinition(chartId);
    expect(model.getters.getOrbexChartDisplayName(chartId)).toBe("(#1) Partners");
    model.dispatch("UPDATE_CHART", {
        definition: {
            ...definition,
            title: { text: "" },
    ...
    expect(model.getters.getOrbexChartDisplayName(chartId)).toBe("(#1) Orbex Line Chart");
});

test("Every Orbex chart type has a default title", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "orbex_line" });
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
    const figureId = model.getters.getFigureIdFromChartId(chartId);
    const chartTypes = chartRegistry.getKeys().filter((type) => type.startsWith("orbex_"));
    model.dispatch("UPDATE_CHART", {
    ...
    for (const chartType of chartTypes) {
        ...
        expect(model.getters.getOrbexChartDisplayName(chartId)).toBe(`(#1) Orbex ${chartName} Chart`);
    }
});
...
test("Orbex line and bar charts display only horizontal grid lines", async () => {
    const { model } = await createSpreadsheetWithChart({
        type: "orbex_line",
    });
    const sheetId = model.getters.getActiveSheetId();
...
test("Can configure the chart datasets", async () => {
    ...
    const { model } = await createSpreadsheetWithChart({
        type: "orbex_bar",
        serverData,
        definition: { type: "orbex_bar", metaData, searchParams, id: "42" },
    });
    await waitForDataLoaded(model);
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
...
test("Non-web chart types are using data source in 'bar' mode", async () => {
    const { model } = await createSpreadsheetWithChart({ type: "orbex_radar" });
    const sheetId = model.getters.getActiveSheetId();
    const chartId = model.getters.getChartIds(sheetId)[0];
