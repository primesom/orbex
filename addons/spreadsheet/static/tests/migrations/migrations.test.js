...
import { load } from "@orbex/o-spreadsheet";
...
test("Orbex formulas are migrated", () => {
    const data = {
        version: 16,
        sheets: [
            {
    ...
    expect(migratedData.sheets[0].cells.A3).toBe(`=ORBEX.FILTER.VALUE.V18("1")`);
    expect(migratedData.sheets[0].cells.A4).toBe(`=ORBEX.LIST("1")`);
    expect(migratedData.sheets[0].cells.A5).toBe(`=ORBEX.LIST.HEADER("1")`);
    expect(migratedData.sheets[0].cells.A6).toBe(`=ORBEX.PIVOT.POSITION("1")`);
...
test("Pivot 'day' arguments are migrated", () => {
    const data = {
        ...
        sheets: [
            {
                cells: {
                    A1: { content: `=ORBEX.PIVOT("1","21/07/2022")` },
                    A2: { content: `=ORBEX.PIVOT.HEADER("1","11/12/2022")` },
                    A3: { content: `=orbex.pivot("1","21/07/2021")` },
                    A4: { content: `=ORBEX.PIVOT("1","test")` },
                    A5: { content: `=orbex.pivot("1","21/07/2021")+"21/07/2021"` },
...
test("Pivot formulas are migrated from 9 to 10", () => {
    const data = {
        ...
        sheets: [
            {
                cells: {
                    A1: { content: `=ORBEX.PIVOT("1")` },
                    A2: { content: `=ORBEX.PIVOT.HEADER("1")` },
                    A3: { content: `=ORBEX.PIVOT.POSITION("1")` },
                    A4: { content: `=ORBEX.PIVOT.TABLE("1")` },
                    A5: { content: `=orbex.pivot("1")` },
                },
            },
        ],
    };
    ...
    expect(migratedData.sheets[0].cells.A1).toBe(`=PIVOT.VALUE("1")`);
    expect(migratedData.sheets[0].cells.A2).toBe(`=PIVOT.HEADER("1")`);
    expect(migratedData.sheets[0].cells.A3).toBe(`=ORBEX.PIVOT.POSITION("1")`);
...
test("Pivot formulas using pivot positions are migrated (11 to 12)", () => {
        ...
        sheets: [
            {
                cells: {
                    A1: {
                        content: `=-PIVOT.VALUE("1","balance","account_id",ORBEX.PIVOT.POSITION("1","account_id",12),"date:quarter","4/"&ORBEX.FILTER.VALUE("Year"))`,
                    },
                    A2: {
                        content: `=PIVOT.HEADER("1","account_id",ORBEX.PIVOT.POSITION("1","account_id",14))`,
                    },
                    A3: { content: `=ORBEX.PIVOT.POSITION("1","account_id",14)` },
                    A4: { content: `=ORBEX.PIVOT.POSITION("1",14)` },
                },
            },
        ],
    };
    ...
    expect(migratedData.sheets[0].cells.A1).toBe(
        `=-PIVOT.VALUE("1","balance","#account_id",12,"date:quarter","4/"&ORBEX.FILTER.VALUE.V18("Year"))`
    );
    expect(migratedData.sheets[0].cells.A2).toBe(`=PIVOT.HEADER("1","#account_id",14)`);
    expect(migratedData.sheets[0].cells.A3).toBe(`=ORBEX.PIVOT.POSITION("1","account_id",14)`);
    expect(migratedData.sheets[0].cells.A4).toBe(`=ORBEX.PIVOT.POSITION("1",14)`);
});
...
test("18.5.10: ORBEX.FILTER.VALUE to ORBEX.FILTER.VALUE.V18 in cells", () => {
    const data = {
        version: "18.4.14",
        sheets: [
            {
                cells: {
                    A1: '=ORBEX.FILTER.VALUE("MyFilter")+orbex.filter.value("AnotherFilter")',
                },
            },
        ],
    };
    const migratedData = load(data);
    expect(migratedData.sheets[0].cells.A1).toBe(
        `=ORBEX.FILTER.VALUE.V18("MyFilter")+ORBEX.FILTER.VALUE.V18("AnotherFilter")`
    );
});
