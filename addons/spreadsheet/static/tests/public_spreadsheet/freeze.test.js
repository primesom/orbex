import { animationFrame, describe, expect, test } from "@orbex/hoot";
import { Model, registries } from "@orbex/o-spreadsheet";
...
import { freezeOrbexData, waitForDataLoaded } from "@spreadsheet/helpers/model";
import { OrbexPivot, OrbexPivotRuntimeDefinition } from "@spreadsheet/pivot/orbex_pivot";
...
test("orbex pivot functions are replaced with their value", async function () {
    ...
    const data = await freezeOrbexData(model);
...
test("Pivot with a type different of ORBEX is not converted", async function () {
    // Add a pivot with a type different of ORBEX
...
test("orbex pivot functions detection is not case sensitive", async function () {
    const { model } = await createSpreadsheetWithPivot();
    setCellContent(model, "A1", '=pivot.value(1,"probability:avg")');
    const data = await freezeOrbexData(model);
...
test("orbex charts are replaced with an image", async function () {
...
test("Carousels figure with orbex data is converted to an image", async function () {
    const { model } = await createSpreadsheetWithChart({ type: "orbex_bar" });
    const sheetId = model.getters.getActiveSheetId();
...
test("Empty ORBEX.LIST result is frozen to an empty string", async function () {
    const { model } = await createSpreadsheetWithList();
    setCellContent(model, "A1", '=ORBEX.LIST(1, 9999,"probability")'); // has no record
    await waitForDataLoaded(model);
    expect(getEvaluatedCell(model, "A1").value).toBe("");
    const frozenData = await freezeOrbexData(model);
    expect(frozenData.sheets[0].cells.A1).toBe('=""');
});

test("orbex links are replaced with their label", async function () {
    const view = {
        name: "an orbex view",
        viewType: "list",
        action: {
    ...
    const data = {
        sheets: [
            {
                cells: {
                    A1: "[menu_xml](orbex://ir_menu_xml_id/test_menu)",
                    A2: "[menu_id](orbex://ir_menu_id/12)",
                    A3: `[orbex_view](orbex://view/${JSON.stringify(view)})`,
                    A4: "[external_link](https://orbexsuite.com)",
    ...
    expect(frozenData.sheets[0].cells.A3).toBe("[orbex_view](neutralized:link)");
    expect(frozenData.sheets[0].cells.A4).toBe("[external_link](https://orbexsuite.com)");
