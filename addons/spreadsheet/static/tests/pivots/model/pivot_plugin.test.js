import { describe, expect, test, Deferred } from "@orbex/hoot";
...
test("evaluates only once when two pivots are loading", async function () {
    ...
    model.config.custom.orbexDataProvider.addEventListener("data-source-updated", () =>
...
test("can edit pivot domain with UPDATE_ORBEX_PIVOT_DOMAIN", async () => {
    const { model } = await createSpreadsheetWithPivot();
    const [pivotId] = model.getters.getPivotIds();
