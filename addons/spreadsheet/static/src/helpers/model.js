...
import { parse, helpers, iterateAstNodes } from "@orbex/o-spreadsheet";
import { isLoadingError } from "@spreadsheet/o_spreadsheet/errors";
import { OrbexSpreadsheetModel } from "@spreadsheet/model";
...
import {
    ...
    isIrMenuXmlUrl,
} from "@spreadsheet/ir_ui_menu/orbex_menu_link_cell";
import { getNeutralizedLink } from "./neutralized_link";

/**
 * @typedef {import("@spreadsheet").OrbexSpreadsheetModel} OrbexSpreadsheetModel
 */
...
export function createSpreadsheetModel({ env, data, revisions }) {
    const orbexDataProvider = new OrbexDataProvider(env);
    const model = new OrbexSpreadsheetModel(data, { custom: { env, orbexDataProvider } }, revisions);
    return model;
}

/**
 * @param {OrbexSpreadsheetModel} model
 */
export async function waitForOrbexSources(model) {
    ...
    promises.push(
        ...model.getters
            .getPivotIds()
            .filter((pivotId) => model.getters.getPivotCoreDefinition(pivotId).type === "ORBEX")
...
export async function waitForDataLoaded(model) {
    await waitForOrbexSources(model);
    const orbexDataProvider = model.config.custom.orbexDataProvider;
    if (!orbexDataProvider) {
        return;
    }
    await new Promise((resolve, reject) => {
        function check() {
            ...
            if (isLoaded(model)) {
                orbexDataProvider.removeEventListener("data-source-updated", check);
                resolve();
            }
        }
        orbexDataProvider.addEventListener("data-source-updated", check);
        check();
    });
}

function containsLinkToOrbex(link) {
    if (link && link.url) {
        return (
 ...
 */
export async function freezeOrbexData(model) {
    ...
    for (const sheet of Object.values(data.sheets)) {
        ...
        for (const [xc, content] of Object.entries(sheet.cells)) {
            ...
            if (containsOrbexFunction(content)) {
                const pivotId = model.getters.getPivotIdFromPosition(position);
                if (pivotId && model.getters.getPivotCoreDefinition(pivotId).type !== "ORBEX") {
                    continue;
                }
            ...
            if (containsLinkToOrbex(evaluatedCell.link)) {
        ...
        for (const figure of sheet.figures) {
            if (
                figure.tag === "chart" &&
                (figure.data.type.startsWith("orbex_") || figure.data.type === "geo")
            ) {
                const img = orbexChartToImage(model, figure, figure.data.chartId);
                figure.tag = "image";
                figure.data = {
                    path: img,
                ...
                const hasImageChart = figure.data.items.some((item) => {
                    ...
                    return (
                        chartDefinition.type.startsWith("orbex_") || chartDefinition.type === "geo"
                    );
                });
                if (hasImageChart) {
                    ...
                    figure.tag = "image";
                    figure.data = {
                        path: orbexChartToImage(model, figure, chartId),
                        size: { width: figure.width, height: figure.height },
                    };
                }
            }
    ...
    if (data.pivots) {
        data.pivots = Object.fromEntries(
            Object.entries(data.pivots).filter(([id, def]) => def.type !== "ORBEX")
        );
    }
    data.lists = {};
 ...
 * @returns {boolean}
 */
function containsOrbexFunction(content) {
    if (
        !content ||
        !content.startsWith("=") ||
        (!content.toUpperCase().includes("ORBEX.") &&
    ...
    try {
        ...
        return iterateAstNodes(ast).some(
            (ast) =>
                ast.type === "FUNCALL" &&
                (ast.value.toUpperCase().startsWith("ORBEX.") ||
 ...
 * @returns {string}
 */
function orbexChartToImage(model, figure, chartId) {
