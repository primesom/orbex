import { EvaluationError, helpers, registries } from "@orbex/o-spreadsheet";
...
export class CurrencyPlugin extends OrbexUIPlugin {
    ...
    constructor(config) {
        ...
        this._serverData = config.custom.orbexDataProvider?.serverData;
    }

    get serverData() {
        if (!this._serverData) {
            throw new Error(
                "'serverData' is not defined, please make sure a 'OrbexDataProvider' instance is provided to the model."
            );
        }
...
featurePluginRegistry.add("orbexCurrency", CurrencyPlugin);
