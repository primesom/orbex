...
export class OrbexChartCoreViewPlugin extends OrbexUIPlugin {
    static getters = /** @type {const} */ (["getChartDataSource", "getOrbexEnv"]);
    ...
    beforeHandle(cmd) {
        switch (cmd.type) {
            case "START":
                for (const chartId of this.getters.getOrbexChartIds()) {
        ...
        switch (cmd.type) {
            ...
            case "REDO": {
                ...
                for (const cmd of domainEditionCommands) {
                    if (!this.getters.getOrbexChartIds().includes(cmd.chartId)) {
                        continue;
                    }
                    if (this._shouldReloadDataSource(cmd.chartId, cmd.definition)) {
                        this._resetChartDataSource(cmd.chartId);
    ...
    getChartDataSource(chartId) {
        const dataSourceId = this._getOrbexChartDataSourceId(chartId);
    ...
    _getOrbexChartDataSourceId(chartId) {
    ...
    _refreshOrbexCharts() {
        for (const chartId of this.getters.getOrbexChartIds()) {
            this._refreshOrbexChart(chartId);
        }
    }
