...
export function onOrbexChartItemClick(getters, chart) {
    return navigateInOrbexMenuOnClick(getters, chart, (chartJsItem, chartData) => {
...
export function onWaterfallOrbexChartItemClick(getters, chart) {
    return navigateInOrbexMenuOnClick(getters, chart, (chartJsItem, chartData) => {
...
export function onGeoOrbexChartItemClick(getters, chart) {
    return navigateInOrbexMenuOnClick(getters, chart, (chartJsItem) => {
...
export function onSunburstOrbexChartItemClick(getters, chart) {
    return navigateInOrbexMenuOnClick(getters, chart, (chartJsItem, chartData, chartJSChart) => {
        const { datasetIndex, index } = chartJsItem;
...
export function onTreemapOrbexChartItemClick(getters, chart) {
    return navigateInOrbexMenuOnClick(getters, chart, (chartJsItem, chartData, chartJSChart) => {
        const { datasetIndex, index } = chartJsItem;
        const rawItem = chartJSChart.data.datasets[datasetIndex].data[index];
...
function navigateInOrbexMenuOnClick(getters, chart, getDomainFromChartItem) {
    return async (event, items, chartJSChart) => {
        const env = getters.getOrbexEnv();
        const { datasets, labels } = chart.dataSource.getData();
...
export function onOrbexChartItemHover() {
    return (event, items) => {
        if (items.length > 0) {
...
export function onGeoOrbexChartItemHover() {
    return (event, items) => {
        if (!items.length) {
            event.native.target.style.cursor = "";
            return;
...
export async function navigateToOrbexMenu(menu, actionService, notificationService, newWindow) {
    if (!menu) {
