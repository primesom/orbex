...
const MAP_V1 = {
    PIVOT: "ORBEX.PIVOT",
    "PIVOT.HEADER": "ORBEX.PIVOT.HEADER",
    "PIVOT.POSITION": "ORBEX.PIVOT.POSITION",
    "FILTER.VALUE": "ORBEX.FILTER.VALUE",
    LIST: "ORBEX.LIST",
    "LIST.HEADER": "ORBEX.LIST.HEADER",
};

const MAP_FN_NAMES_V10 = {
    "ORBEX.PIVOT": "PIVOT.VALUE",
    "ORBEX.PIVOT.HEADER": "PIVOT.HEADER",
    "ORBEX.PIVOT.TABLE": "PIVOT",
};
...
migrationStepRegistry.add("17.3.1", {
    migrate(data) {
        return migrateOrbexData(data);
    },
});
migrationStepRegistry.add("18.1.2", {
    migrate(data) {
        const version = data.orbexVersion || 0;
        if (version < 13) {
    ...
    migrate(data) {
        for (const sheet of data.sheets || []) {
            for (const figure of sheet.figures || []) {
                if (
                    figure.tag === "chart" &&
                    ["orbex_bar", "orbex_line", "orbex_pie"].includes(figure.data.type) &&
...
migrationStepRegistry.add("18.5.10", {
    migrate(data) {
        ...
        const re = /ORBEX\.FILTER\.VALUE/gi
        for (const sheet of data.sheets || []) {
            for (const xc in sheet.cells || {}) {
                const content = sheet.cells[xc];
                if (re.test(content)) {
                    sheet.cells[xc] = content.replaceAll(re, "ORBEX.FILTER.VALUE.V18");
                }
            }
        }
        return data;
...
function migrateOrbexData(data) {
    const version = data.orbexVersion || 0;
    if (version < 1) {
...
function migratePivotDaysParameters(formulaString) {
    ...
    const convertedAst = convertAstNodes(ast, "FUNCALL", (ast) => {
        if (["ORBEX.PIVOT", "ORBEX.PIVOT.HEADER"].includes(ast.value.toUpperCase())) {
...
/**
 * Migrate the pivot data to add the type, by default "ORBEX". And replace the
...
function migrate11to12(data) {
    // remove the calls to ORBEX.PIVOT.POSITION and replace
    ...
    for (const sheet of data.sheets || []) {
        for (const xc in sheet.cells || []) {
            ...
            if (
                ...
                cell.content.includes("ORBEX.PIVOT.POSITION")
            ) {
                const tokens = tokenize(cell.content);
                /* given that orbex.pivot.position is automatically set, we know that:
                1) it is always on the form of ORBEX.PIVOT.POSITION(1, ...)
                2) it is always preceded by a dimension of a pivot or header, inside another pivot formula
                3) there is only one orbex.pivot.position per cell
                4) orbex.pivot.position can only exist after the 3rd token and needs at least 7 tokens to be valid*/
                for (let i = 2; i < tokens.length - 7; i++) {
                    const token = tokens[i];
                    if (
                        token.type === "SYMBOL" &&
                        token.value.toUpperCase() === "ORBEX.PIVOT.POSITION"
                    ) {
                        const order = tokens[i + 6];
                        tokens[i - 2].value = '"#' + tokens[i - 2].value.slice(1); // "dimension" becomes "#dimension"
                        tokens.splice(i, 7); // remove "ORBEX.PIVOT.POSITION", "(", "1", ",", "dimension", ", ", order
                        // tokens[i-1] is the comma before orbex.pivot.position
                        tokens[i] = order;
                        cell.content = tokensToString(tokens);
                    }
                }
...
/**
 * Change pivot sortedColumn of the orbex's pivot model to the sortedColumn of o-spreadsheet
 */
