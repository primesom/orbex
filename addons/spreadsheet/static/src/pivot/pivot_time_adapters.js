...
import { registries, helpers, constants, EvaluationError } from "@orbex/o-spreadsheet";
...
function boundedOrbexNumberDateAdapter(lower, upper) {
    return {
...
const orbexMinuteNumberAdapter = {
    normalizeServerValue(groupBy, field, readGroupResult) {
        return Number(readGroupResult[groupBy]);
    },
    increment(normalizedValue, step) {
...
const orbexSecondNumberAdapter = {
    normalizeServerValue(groupBy, field, readGroupResult) {
        return Number(readGroupResult[groupBy]);
    },
    increment(normalizedValue, step) {
...
pivotTimeAdapterRegistry.add("week", falseHandlerDecorator(orbexWeekAdapter));
pivotTimeAdapterRegistry.add("quarter", falseHandlerDecorator(orbexQuarterAdapter));

extendSpreadsheetAdapter("day", orbexDayAdapter);
extendSpreadsheetAdapter("year", orbexYearAdapter);
extendSpreadsheetAdapter("day_of_month", boundedOrbexNumberDateAdapter(1, 31));
extendSpreadsheetAdapter("iso_week_number", boundedOrbexNumberDateAdapter(0, 54));
extendSpreadsheetAdapter("month_number", boundedOrbexNumberDateAdapter(1, 12));
extendSpreadsheetAdapter("quarter_number", boundedOrbexNumberDateAdapter(1, 4));
extendSpreadsheetAdapter("day_of_week", orbexDayOfWeekAdapter);
extendSpreadsheetAdapter("hour_number", orbexHourNumberAdapter);
extendSpreadsheetAdapter("minute_number", orbexMinuteNumberAdapter);
extendSpreadsheetAdapter("second_number", orbexSecondNumberAdapter);
extendSpreadsheetAdapter("month", orbexMonthAdapter);
