...
import { NO_RECORD_AT_THIS_POSITION, OrbexPivotModel } from "./pivot_model";
import { EvaluationError, PivotRuntimeDefinition, registries, helpers } from "@orbex/o-spreadsheet";
import { LOADING_ERROR } from "@spreadsheet/data_sources/data_source";
import { omit } from "@web/core/utils/objects";
import { OrbexPivotLoader } from "./orbex_pivot_loader";
...
/**
 * @typedef {import("@orbex/o-spreadsheet").FunctionResultObject} FunctionResultObject
 * @typedef {import("@orbex/o-spreadsheet").PivotMeasure} PivotMeasure
 * @typedef {import("@orbex/o-spreadsheet").PivotDomain} PivotDomain
 * @typedef {import("@orbex/o-spreadsheet").PivotDimension} PivotDimension
 * @typedef {import("@orbex/o-spreadsheet").PivotCoreMeasure} PivotCoreMeasure
 * @typedef {import("@spreadsheet").WebPivotModelParams} WebPivotModelParams
 * @typedef {import("@spreadsheet").OrbexPivot<OrbexPivotRuntimeDefinition>} IPivot
 * @typedef {import("@spreadsheet").OrbexFields} OrbexFields
 * @typedef {import("@spreadsheet").OrbexPivotCoreDefinition} OrbexPivotCoreDefinition
 * @typedef {import("@spreadsheet").OrbexGetters} OrbexGetters
 * @typedef {import("@spreadsheet/data_sources/orbex_data_provider").OrbexDataProvider} OrbexDataProvider
 */

/**
 * @implements {IPivot}
 */
export class OrbexPivot {
    /**
     *
     * @override
     * @param {Object} services Services (see DataSource)
     * @param {Object} params
     * @param {OrbexPivotCoreDefinition} params.definition
     * @param {OrbexGetters} params.getters
     */
    constructor(services, { definition, getters }) {
        /** @type {"ORBEX"} */
        this.type = "ORBEX";

        /** @type {OrbexPivotCoreDefinition} @protected */
        this.coreDefinition = definition;

        this.needsReevaluation = false;

        /** @type {OrbexPivotRuntimeDefinition | undefined} @protected */
        this.runtimeDefinition = undefined;

        /** @type {OrbexPivotModel | undefined} @protected */
        this.model = undefined;

        /** @type {OrbexGetters} @protected */
        this.getters = getters;

        /** @protected */
        this.loader = new OrbexPivotLoader(services.orbexDataProvider, this._load.bind(this));

        /** @type {OrbexFields | undefined} @protected */
        this._fields = undefined;

        /** @protected @type {OrbexDataProvider}*/
        this.orbexDataProvider = services.orbexDataProvider;
    ...
    /**
     * @param {OrbexPivotCoreDefinition} nextDefinition
     */
    onDefinitionChange(nextDefinition) {
        ...
        if (
            ...
            if (
                ...
                this.coreDefinition = nextDefinition;
                const runtimeDefinition = new OrbexPivotRuntimeDefinition(
                    this.coreDefinition,
                    this.getFields()
                );
    ...
    async createModelAndDefinition() {
        ...
        const definition = new OrbexPivotRuntimeDefinition(this.coreDefinition, this.getFields());
        const model = new OrbexPivotModel(
            { _t },
            {
            ...
            {
                orm: this.orbexDataProvider.orm,
                serverData: this.orbexDataProvider.serverData,
    ...
    /**
     * @param {import("@orbex/o-spreadsheet").Maybe<FunctionResultObject>[]} args
     *
     * @returns {PivotDomain}
     */
        ...
        await Promise.all(
            related.map((dimension) =>
                getRelationalFieldDefinition(
                    ...
                    dimension.fieldName,
                    this.orbexDataProvider.fieldService
    ...
    async _loadPropertiesDefinitions() {
        ...
        const orm = this.orbexDataProvider.orm;
...
export class OrbexPivotRuntimeDefinition extends PivotRuntimeDefinition {
    /**
     * @param {OrbexPivotCoreDefinition} definition
     * @param {OrbexFields} fields
     */
...
supportedPivotPositionalFormulaRegistry.add("ORBEX", true);
