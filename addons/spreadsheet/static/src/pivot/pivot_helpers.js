...
import { EvaluationError, helpers } from "@orbex/o-spreadsheet";

const { isDateOrDatetimeField } = helpers;

/**
 * @typedef {import("@orbex/o-spreadsheet").Token} Token
 * @typedef {import("@orbex/o-spreadsheet").Granularity} Granularity
 * */
...
export const ORBEX_AGGREGATORS = {};

for (const type in AGGREGATORS_BY_FIELD_TYPE) {
    ORBEX_AGGREGATORS[type] = {};
    for (const aggregator of AGGREGATORS_BY_FIELD_TYPE[type]) {
        ORBEX_AGGREGATORS[type][aggregator] = AGGREGATOR_NAMES[aggregator];
    }
}
...
/**
 * @typedef {import("@spreadsheet").OrbexField} OrbexField
 */

/**
 * Parses the positional char (#), the field and operator string of pivot group.
 * e.g. "create_date:month"
 * @param {Record<string, OrbexField | undefined>} allFields
 * @param {string} groupFieldString
 * @returns {{field: OrbexField, granularity: Granularity, isPositional: boolean, dimensionWithGranularity: string}}
 */
