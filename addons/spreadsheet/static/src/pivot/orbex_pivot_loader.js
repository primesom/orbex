...
import { EvaluationError, CellErrorType } from "@orbex/o-spreadsheet";
...
export class OrbexPivotLoader {
    /**
     * @param {OrbexDataProvider} orbexDataProvider
     * @param {Function} load Function to fetch data
     */
    constructor(orbexDataProvider, load) {
        /** @private @type {OrbexDataProvider} */
        this.orbexDataProvider = orbexDataProvider;
        /** @protected */
        this.loadFn = load;
    ...
    async load(options) {
        if (options && options.reload) {
            this.orbexDataProvider.cancelPromise(this.loadPromise);
            this.loadPromise = undefined;
        }
        if (!this.loadPromise) {
            this.isFullyLoaded = false;
            ...
            await this.orbexDataProvider.notifyWhenPromiseResolves(this.loadPromise);
        }
     ...
     * @returns {Promise<OrbexFields>} Fields of the model
     */
    async getFields(model) {
        return getFields(this.orbexDataProvider.fieldService, model);
    }
    /**
     * @param {string} model Technical name of the model
    ...
    async getModelLabel(model) {
        const result = await this.orbexDataProvider.orm
            .cache({ type: "disk" })
