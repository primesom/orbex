 ...
 * @typedef {import("@orbex/o-spreadsheet").FPayload} FPayload
 */
...
const ORBEX_FILTER_VALUE = /** @satisfies {CustomFunctionDescription} */ ({
...
// ORBEX.FILTER.VALUE.V18

const ORBEX_FILTER_VALUE_V18 = /** @satisfies {CustomFunctionDescription} */ ({
    description: _t("Compatibility version of ORBEX.FILTER.VALUE for v18 spreadsheets. Required for date filters. Optional for others."),
    args: [arg("filter_name (string)", _t("The label of the filter whose value to return."))],
    category: "Orbex",
    hidden: true,
    compute: function (filterName) {
        const filter = this.getters.getGlobalFilterByName(toString(filterName, this.locale));
        const value = this["ORBEX.FILTER.VALUE"](filterName);
        if (filter?.type === "relation") {
            ...
            const result =  this.orbexDataProvider.serverData.get(filter.modelName, "web_search_read", [
                [["id", "in", ids]],
                { display_name: {} },
            ])
...
functionRegistry
    .add("ORBEX.FILTER.VALUE", ORBEX_FILTER_VALUE)
    .add("ORBEX.FILTER.VALUE.V18", ORBEX_FILTER_VALUE_V18);
