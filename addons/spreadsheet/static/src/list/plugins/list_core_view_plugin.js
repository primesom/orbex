...
import { OrbexCoreViewPlugin } from "@spreadsheet/plugins";
...
export class ListCoreViewPlugin extends OrbexCoreViewPlugin {
    ...
    handle(cmd) {
        switch (cmd.type) {
            ...
            case "SET_GLOBAL_FILTER_VALUE":
                this._addDomains();
                break;
            case "UPDATE_ORBEX_LIST":
            case "UPDATE_ORBEX_LIST_DOMAIN": {
            ...
            case "REDO": {
                ...
                const updateCommands = cmd.commands.filter(
                    (cmd) =>
                        cmd.type === "UPDATE_ORBEX_LIST_DOMAIN" ||
                        cmd.type === "UPDATE_ORBEX_LIST" ||
                        cmd.type === "INSERT_ORBEX_LIST"
                );
    ...
    getListFieldFromPosition(position) {
        ...
        const fieldArg = functionName === "ORBEX.LIST.HEADER" ? args[1] : args[2];
    ...
    isSortableListHeader(position) {
        ...
        const dataSource = this.getters.getListDataSource(listId);
        return (
            functionName === "ORBEX.LIST.HEADER" &&
