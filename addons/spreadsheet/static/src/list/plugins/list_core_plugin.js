...
export class ListCorePlugin extends OrbexCorePlugin {
    ...
    _insertHeaders(sheetId, anchor, id, columns) {
        ...
        for (const column of columns) {
            const content = column.string
                ? `=ORBEX.LIST.HEADER(${id},"${column.name}","${column.string}")`
                : `=ORBEX.LIST.HEADER(${id},"${column.name}")`;
        ...
        for (let i = 1; i <= linesNumber; i++) {
            ...
            for (const column of columns) {
                this.dispatch("UPDATE_CELL", {
                    ...
                    col,
                    row,
                    content: `=ORBEX.LIST(${id},${i},"${column.name}")`,
                });
                col++;
            }
            row++;
