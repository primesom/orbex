...
OutFile "${TOOLSDIR}\server\orbex_iot_setup_${VERSION}.exe"
...
; English
LangString DESC_orbex_IoT ${LANG_ENGLISH} "Install the orbex Server with IoT modules."
LangString DESC_FinishPage_Link ${LANG_ENGLISH} "Contact orbex for Partnership and/or Support"
LangString TITLE_orbex_IoT ${LANG_ENGLISH} "orbex IoT"
...
; French
LangString DESC_orbex_IoT ${LANG_FRENCH} "Installation du Serveur orbex avec les modules IoT."
LangString DESC_FinishPage_Link ${LANG_FRENCH} "Contactez orbex pour un Partenariat et/ou du Support"
LangString TITLE_orbex_IoT ${LANG_FRENCH} "orbex IoT"
...
Section $(TITLE_orbex_IoT) Sectionorbex_IoT
    ...
    # Cloning orbex
    DetailPrint "Cloning orbex"
    nsExec::Exec '"$INSTDIR\git\cmd\git.exe" clone --filter=tree:0 -b 19.0 --single-branch --no-checkout https://github.com/orbex/orbex.git "$INSTDIR\orbex"'

    DetailPrint "Configuring Sparse Checkout for IoT modules"
    nsExec::Exec '"$INSTDIR\git\cmd\git.exe" -C "$INSTDIR\orbex" sparse-checkout init --no-cone'
    nsExec::Exec '"$INSTDIR\git\cmd\git.exe" -C "$INSTDIR\orbex" sparse-checkout set addons/web addons/hw_* addons/iot_drivers addons/iot_base addons/iot_box_image/configuration addons/point_of_sale/tools/posbox/configuration/requirements.txt orbex orbex-bin'

    DetailPrint "Checking out the repository"
    nsExec::Exec '"$INSTDIR\git\cmd\git.exe" -C "$INSTDIR\orbex" checkout'
    ...
    DetailPrint "Configuring $(TITLE_orbex_IoT)"
    # Set data_dir
    WriteIniStr "$INSTDIR\orbex.conf" "options" "data_dir" "$INSTDIR\sessions"
    # set modules to load
    WriteIniStr "$INSTDIR\orbex.conf" "options" "server_wide_modules" "iot_drivers,web"
    # Configure logging
    WriteIniStr "$INSTDIR\orbex.conf" "options" "logfile" "$INSTDIR\orbex.log"
    WriteIniStr "$INSTDIR\orbex.conf" "options" "log_handler" ":WARNING"
    WriteIniStr "$INSTDIR\orbex.conf" "options" "log_level" "warn"
    # Other configuration
    WriteIniStr "$INSTDIR\orbex.conf" "options" "list_db" "False"
    WriteIniStr "$INSTDIR\orbex.conf" "options" "max_cron_threads" "0"
    # Write VERSION file
    ...
    nsExec::ExecToLog '"$INSTDIR\nssm\win64\nssm.exe" set ${SERVICENAME} AppParameters "\"$INSTDIR\orbex\orbex-bin\" -c "\"$INSTDIR\orbex.conf\"'
    nsExec::ExecToLog '"$INSTDIR\nssm\win64\nssm.exe" set ${SERVICENAME} ObjectName "LOCALSERVICE"'
    AccessControl::GrantOnFile  "$INSTDIR" "LOCALSERVICE" "FullAccess"

    nsExec::ExecToStack '"$INSTDIR\python\python.exe" "$INSTDIR\orbex\orbex-bin" genproxytoken'
    pop $0
    pop $ProxyTokenPwd
    WriteIniStr "$INSTDIR\orbex.conf" "options" "proxy_access_token" "$ProxyTokenPwd"
...
Section -$(TITLE_Nginx) Nginx
    ...
    File "..\..\orbex\addons\iot_box_image\overwrite_after_init\etc\ssl\certs\nginx-cert.crt"
    File "..\..\orbex\addons\iot_box_image\overwrite_after_init\etc\ssl\private\nginx-cert.key"
SectionEnd
...
!insertmacro MUI_FUNCTION_DESCRIPTION_BEGIN
    !insertmacro MUI_DESCRIPTION_TEXT ${Sectionorbex_IoT} $(DESC_orbex_IoT)
...
Function .onInit
    ...
    ${If} $previous_install_dir == ""
        StrCpy $INSTDIR "$PROGRAMFILES64\orbex IoT ${VERSION}"
