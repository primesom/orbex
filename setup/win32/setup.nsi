...
!ifndef SERVICENAME
	!define SERVICENAME 'orbex-server-${VERSION}'
!endif

!ifndef TOOLSDIR
	!define TOOLSDIR 'c:\orbexbuild'
!endif

!define PRODUCT_NAME "orbex"
...
!define UNINSTALL_REGISTRY_KEY_SERVER "${UNINSTALL_BASE_REGISTRY_KEY}\orbex Server ${VERSION}"
...
OutFile "${TOOLSDIR}\server\orbex_setup_${VERSION}.exe"
...
!define MUI_ICON "${PIXMAPS_PATH}\orbex-icon.ico"

!define MUI_WELCOMEFINISHPAGE_BITMAP "${PIXMAPS_PATH}\orbex-intro.bmp"
!define MUI_UNWELCOMEFINISHPAGE_BITMAP "${PIXMAPS_PATH}\orbex-intro.bmp"
!define MUI_HEADERIMAGE
!define MUI_HEADERIMAGE_BITMAP "${PIXMAPS_PATH}\orbex-slogan.bmp"
...
!define MUI_FINISHPAGE_LINK_LOCATION "https://www.orbexsuite.com/page/contactus"
...
; English
LangString DESC_orbex_Server ${LANG_ENGLISH} "Install the orbex Server with all the orbex standard modules."
LangString DESC_PostgreSQL ${LANG_ENGLISH} "Install the PostgreSQL RDBMS used by orbex."
LangString DESC_FinishPage_Link ${LANG_ENGLISH} "Contact orbex for Partnership and/or Support"
LangString DESC_AtLeastOneComponent ${LANG_ENGLISH} "You have to choose at least one component"
LangString DESC_CanNotInstallPostgreSQL ${LANG_ENGLISH} "You can not install the PostgreSQL database without the orbex Server"
...
LangString Profile_AllInOne ${LANG_ENGLISH} "orbex Server And PostgreSQL Server"
LangString Profile_Server ${LANG_ENGLISH} "orbex Server Only"
LangString TITLE_orbex_Server ${LANG_ENGLISH} "orbex Server"
LangString TITLE_PostgreSQL ${LANG_ENGLISH} "PostgreSQL Database"
LangString DESC_FinishPageText ${LANG_ENGLISH} "Start orbex"
LangString UnsafeDirText ${LANG_ENGLISH} "Installing outside of $PROGRAMFILES64 is not recommended.$\nDo you want to continue ?"

; French
LangString DESC_orbex_Server ${LANG_FRENCH} "Installation du Serveur orbex avec tous les modules orbex standards."
LangString DESC_PostgreSQL ${LANG_FRENCH} "Installation de la base de données PostgreSQL utilisée par orbex."
LangString DESC_FinishPage_Link ${LANG_FRENCH} "Contactez orbex pour un Partenariat et/ou du Support"
LangString DESC_AtLeastOneComponent ${LANG_FRENCH} "Vous devez choisir au moins un composant"
LangString DESC_CanNotInstallPostgreSQL ${LANG_FRENCH} "Vous ne pouvez pas installer la base de données PostgreSQL sans le serveur orbex"
...
LangString Profile_AllInOne ${LANG_FRENCH} "Serveur orbex Et Serveur PostgreSQL"
LangString Profile_Server ${LANG_FRENCH} "Seulement Le Serveur orbex"
LangString TITLE_orbex_Server ${LANG_FRENCH} "Serveur orbex"
LangString TITLE_PostgreSQL ${LANG_FRENCH} "Installation du serveur de base de données PostgreSQL"
LangString DESC_FinishPageText ${LANG_FRENCH} "Démarrer orbex"
...
Section $(TITLE_orbex_Server) Sectionorbex_Server
    SectionIn 1 2
    ...
    # If there is a previous install of the orbex Server, keep the login/password from the config file
    WriteIniStr "$INSTDIR\server\orbex.conf" "options" "db_host" $TextPostgreSQLHostname
    WriteIniStr "$INSTDIR\server\orbex.conf" "options" "db_user" $TextPostgreSQLUsername
    WriteIniStr "$INSTDIR\server\orbex.conf" "options" "db_password" $TextPostgreSQLPassword
    WriteIniStr "$INSTDIR\server\orbex.conf" "options" "db_port" $TextPostgreSQLPort
    # Fix the addons path
    WriteIniStr "$INSTDIR\server\orbex.conf" "options" "addons_path" "$INSTDIR\server\orbex\addons"
    WriteIniStr "$INSTDIR\server\orbex.conf" "options" "bin_path" "$INSTDIR\thirdparty"
    ...
    ${If} $HasPostgreSQL == 0
        WriteIniStr "$INSTDIR\server\orbex.conf" "options" "pg_path" "$INSTDIR\PostgreSQL\bin"
    ${EndIf}

    # Productivity Apps
    WriteIniStr "$INSTDIR\server\orbex.conf" "options" "default_productivity_apps" "True"
    DetailPrint "Installing Windows service"
    nsExec::ExecTOLog '"$INSTDIR\python\python.exe" "$INSTDIR\server\orbex-bin" --stop-after-init -c "$INSTDIR\server\orbex.conf" --logfile "$INSTDIR\server\orbex.log" -s'
    nsExec::ExecToLog '"$INSTDIR\nssm\win64\nssm.exe" install ${SERVICENAME} "$INSTDIR\python\python.exe"'
    nsExec::ExecToLog '"$INSTDIR\nssm\win64\nssm.exe" set ${SERVICENAME} AppDirectory "$\"$INSTDIR\python$\""'
    nsExec::ExecToLog '"$INSTDIR\nssm\win64\nssm.exe" set ${SERVICENAME} AppParameters "\"$INSTDIR\server\orbex-bin\" -c "\"$INSTDIR\server\orbex.conf\"'

    Call RestartorbexService
SectionEnd
...
Section $(TITLE_PostgreSQL) SectionPostgreSQL
    ...
    ExecWait '"$TEMP\$postgresql_exe_filename" \
        ...
        --servicename "PostgreSQL_For_orbex" \
...
Section -Post
    ...
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "HelpLink" "support@orbexsuite.com"
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "HelpTelephone" "+32.81.81.37.00"
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "URLInfoAbout" "https://www.orbexsuite.com"
    WriteRegStr HKLM       "${UNINSTALL_REGISTRY_KEY}" "Contact" "sales@orbexsuite.com"
...
!insertmacro MUI_FUNCTION_DESCRIPTION_BEGIN
    !insertmacro MUI_DESCRIPTION_TEXT ${Sectionorbex_Server} $(DESC_orbex_Server)
...
Function .onInit
    ...
    ${If} $previous_install_dir == ""
        StrCpy $INSTDIR "$PROGRAMFILES64\orbex ${VERSION}"
...
Function ComponentLeave
    SectionGetFlags ${Sectionorbex_Server} $0
...
Function RestartorbexService
    DetailPrint "Restarting orbex Service"
