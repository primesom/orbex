...
version_info = ...
nt_service_name = ...
exec(open(os.path.join(ROOTDIR, 'orbex', 'release.py'), 'rb').read())
...
DOCKERUSER = """
RUN mkdir /var/lib/orbex && \
    groupadd -g %(group_id)s orbex && \
    useradd -u %(user_id)s -g orbex orbex -d /var/lib/orbex && \
    mkdir /data && \
    chown orbex:orbex /var/lib/orbex /data
USER orbex
""" % {'group_id': os.getgid(), 'user_id': os.getuid()}


class orbexTestTimeoutError(Exception):
    pass


class orbexTestError(Exception):
    pass
...
def _rpc_count_modules(addr='http://127.0.0.1', port=8069, dbname='mycompany'):
    ...
    if len(modules) > 1:
        ...
        if toinstallmodules:
            ...
            raise orbexTestError("Installation of package failed")
        else:
        ...
        raise orbexTestError("Package test: FAILED. Not able to install base.")
...
def publish(args, pub_type, extensions):
    ...
    for extension in extensions:
        release = glob("%s/orbex_*.%s" % (args.build_dir, extension))
        if release:
...
def _prepare_build_dir(args, win32=False, move_addons=True):
    ...
    run_cmd(cmd + ['%s/' % args.orbex_dir, args.build_dir])
    if not move_addons:
        return
    for addon_path in glob(os.path.join(args.build_dir, 'addons/*')):
        if args.blacklist is None or os.path.basename(addon_path) not in args.blacklist:
            try:
                shutil.move(addon_path, os.path.join(args.build_dir, 'orbex/addons'))
...
class Docker():
    ...
    def __init__(self, args):
        ...
        """
        self.args = args
        self.tag = 'orbex-%s-%s-nightly-tests' % (DOCKERVERSION, self.arch)
        ...
        self.docker_template = Path(docker_templates[self.arch]).read_text(encoding='utf-8').replace('USER orbex', DOCKERUSER)
    ...
    def run(self, cmd, build_dir, container_name, user='orbex', exposed_port=None, detach=False, timeout=None):
    ...
    def test_orbex(self):
        logging.info('Starting to test orbex install test')
        start_time = time.time()
        while self.is_running() and (time.time() - start_time) < INSTALL_TIMEOUT:
            time.sleep(5)  # give some time for orbex to install and start
            if os.path.exists(os.path.join(args.build_dir, 'orbex.pid')):
                try:
        ...
        if self.is_running():
            self.stop()
            raise orbexTestTimeoutError('orbex pid file never appeared after %s sec' % INSTALL_TIMEOUT)
        raise orbexTestError('Error while installing/starting orbex after %s sec.\nSee testlogs.txt in build dir' % int(time.time() - start_time))
...
class DockerTgz(Docker):
    ...
    def build(self):
        ...
        self.run('python3 setup.py sdist --quiet --formats=gztar,zip', self.args.build_dir, 'orbex-src-build-%s' % TSTAMP)
        os.rename(glob('%s/dist/orbex-*.tar.gz' % self.args.build_dir)[0], '%s/orbex_%s.%s.tar.gz' % (self.args.build_dir, VERSION, TSTAMP))
        os.rename(glob('%s/dist/orbex-*.zip' % self.args.build_dir)[0], '%s/orbex_%s.%s.zip' % (self.args.build_dir, VERSION, TSTAMP))
    ...
    def start_test(self):
        ...
        cmds = [
            ...
            'su postgres -s /bin/bash -c "createuser -s orbex"',
            'su orbex -s /bin/bash -c "python3 -m venv /var/lib/orbex/orbexvenv"',
            'su orbex -s /bin/bash -c "/var/lib/orbex/orbexvenv/bin/python3 -m pip install --upgrade pip"',
            'su orbex -s /bin/bash -c "/var/lib/orbex/orbexvenv/bin/python3 -m pip install -r /opt/release/requirements.txt"',
            f'su orbex -s /bin/bash -c "/var/lib/orbex/orbexvenv/bin/python3 -m pip install /data/src/orbex_{VERSION}.{TSTAMP}.tar.gz"',
            'su orbex -s /bin/bash -c "createdb mycompany"',
            'su orbex -s /bin/bash -c "/var/lib/orbex/orbexvenv/bin/orbex -d mycompany -i base --stop-after-init"',
            'su orbex -s /bin/bash -c "/var/lib/orbex/orbexvenv/bin/orbex -d mycompany --pidfile=/data/src/orbex.pid"',
        ]
        self.run(' && '.join(cmds), self.args.build_dir, 'orbex-src-test-%s' % TSTAMP, user='root', detach=True, exposed_port=8069, timeout=300)
        self.test_orbex()
...
class DockerDeb(Docker):
    ...
    def build(self):
        ...
        cmds = ["sed -i '1s/^.*$/orbex (%s.%s) stable; urgency=low/' debian/changelog" % (VERSION, TSTAMP)]
        ...
        cmds.append('mv ../orbex_* ./')
        self.run(' && '.join(cmds), self.args.build_dir, 'orbex-deb-build-%s' % TSTAMP)
    ...
    def start_test(self):
        ...
        cmds = [
            ...
            f'/usr/bin/apt-get install -y /data/src/orbex_{VERSION}.{TSTAMP}_all.deb',
            'su orbex -s /bin/bash -c "orbex -d mycompany -i base --pidfile=/data/src/orbex.pid"',
        ]
        self.run(' && '.join(cmds), self.args.build_dir, 'orbex-deb-test-%s' % TSTAMP, user='root', detach=True, exposed_port=8069, timeout=300)
        self.test_orbex()
...
class DockerRpm(Docker):
    ...
    def build(self):
        ...
        rpmbuild_dir = '/var/lib/orbex/rpmbuild'
        build_date = datetime.now().strftime('%a %b %d %Y')
        cmds = [
            'cd /data/src',
            'mkdir -p dist',
            'rpmdev-setuptree -d',
            f'cp -a /data/src/setup/rpm/orbex.spec {rpmbuild_dir}/SPECS/',
            f'tar --transform "s/^\\./orbex-{VERSION}/" -c -z -f {rpmbuild_dir}/SOURCES/orbex-{VERSION}.tar.gz .',
            f'rpmbuild -bb --define="%version {VERSION}" --define "%release {TSTAMP}" --define "%build_date {build_date}" /data/src/setup/rpm/orbex.spec',
            f'mv {rpmbuild_dir}/RPMS/noarch/orbex*.rpm /data/src/dist/'
        ]
        self.run(' && '.join(cmds), self.args.build_dir, f'orbex-rpm-build-{TSTAMP}')
        os.rename(glob('%s/dist/orbex-*.noarch.rpm' % self.args.build_dir)[0], '%s/orbex_%s.%s.rpm' % (self.args.build_dir, VERSION, TSTAMP))
    ...
    def start_test(self):
        ...
        cmds = [
            ...
            'sleep 5',
            'su postgres -c "createuser -s orbex"',
            'su orbex -c "createdb mycompany"',
            'dnf install -q /data/src/orbex_%s.%s.rpm -y' % (VERSION, TSTAMP),
            'su orbex -s /bin/bash -c "orbex -c /etc/orbex/orbex.conf -d mycompany -i base --stop-after-init"',
            'su orbex -s /bin/bash -c "orbex -c /etc/orbex/orbex.conf -d mycompany --pidfile=/data/src/orbex.pid"',
        ]
        self.run(' && '.join(cmds), args.build_dir, 'orbex-rpm-test-%s' % TSTAMP, user='root', detach=True, exposed_port=8069, timeout=300)
        self.test_orbex()
    ...
    def gen_rpm_repo(self, args, rpm_filepath):
        ...
        self.run('createrepo /data/src/', temp_path, 'orbex-rpm-createrepo-%s' % TSTAMP)
...
class DockerWine(Docker):
    ...
    def __init__(self, args):
        ...
        self.nsi_filepath = r"c:\orbexbuild\server\setup\win32\setup.nsi"
    ...
    def build(self):
        ...
        container_python = '/var/lib/orbex/.wine/drive_c/orbexbuild/WinPy64/python-3.12.3.amd64/python.exe'
        ...
        self.run(' && '.join(cmds), self.args.build_dir, 'orbex-win-build-%s' % TSTAMP)
...
class DockerIot(DockerWine):
    ...
    def __init__(self, args):
        ...
        self.nsi_filepath = r"c:\orbexbuild\server\setup\win32\setup-iot.nsi"
        self.nt_service_name = "orbex-iot"

    def build_image(self):
        shutil.copy(os.path.join(self.args.build_dir, 'orbex/addons/iot_box_image/configuration/requirements.txt'), self.docker_dir / 'requirements-iot.txt')
